"""
Tests for the Bambu AMS unit, extras/AFC_BambuAMS.py.

The scan and measurement paths, lane and slot sync, Spoolman binding, the
follower, fault handling, the dryer commands, and the load/unload flows driven
end to end. The bridge that talks to the Pico is tested separately, in
tests/test_AFC_BambuAMS_bridge.py.
...
#
# was tests/test_AFC_bambu_ams.py
# ── prep logo alignment ─────────────────────────────────────────────────────────
...
class TestRecover:
    ...
    def _shim(self, present=True):
        ...
        lane = types.SimpleNamespace(name="lane1", tool_loaded=True,
                                     ...
                                     hub_obj=None)
        shim = types.SimpleNamespace(
            name="BambuAMS_1", logger=_Logger_ams(), _bridge=object(),
...
class TestLoadRecover:
    ...
    def _shim(attempts):
        ...
        shim = types.SimpleNamespace(
            name="AMS", logger=_Logger_ams(), _bridge=object(), afc=afc,
...
def _eject_shim():
    order = []
    shim = types.SimpleNamespace(
        name="AMS", logger=_Logger_ams(), _bridge=object(),
...
def _surface_self(saves=None):
    ...
    return types.SimpleNamespace(
        name="AMS", logger=_Logger_ams(), afc=None,
...
class TestAMissIsRememberedOnBothBranches:
    ...
    def _u(self, found=None):
        u = types.SimpleNamespace(
            name="AMS", logger=_Logger_ams(), _bound_uid={},
...
class TestTheSpoolmanBindingFollowsTheTag:
    ...
    def _u(self, bound_uid=None, spool_id=None):
        u = types.SimpleNamespace(
            name="HT", logger=_Logger_ams(), afc=None,
    ...
    def _shim(self, fail=None):
        ...
        class _B:
            def send(self, obj):
                ...
                sent.append(obj)
        shim = types.SimpleNamespace(
            name="AMS", logger=_Logger_ams(), _bridge=_B(), ams_index=0,
...
class TestFaultHoldCannotLatchForever:
    ...
    def _u(self, paused=False, printing=False, saw_pause=False):
        return types.SimpleNamespace(
            name="BambuAMS_1", logger=_Logger_ams(),
...
class TestPathAdoptionNeedsOnlyOneLoad:
    ...
    def _shim(self, measured, bowden=3000.0):
        writes = []
        shim = types.SimpleNamespace(
            name="BambuAMS_2", logger=_Logger_ams(),
...
class TestTheSensorReadIsThePin:
    ...
    def test_the_pin_beats_a_stale_cache(self):
        shim = types.SimpleNamespace(name="AMS", logger=_Logger_ams())
...
class TestBiteThenLetItPull:
    ...
    def _rig(self, tool_stn=40.0, bite=2.0, settle=1.5):
        ...
        shim = types.SimpleNamespace(
            name="AMS", logger=_Logger_ams(), afc=afc,
...
class TestWaitForTheUnitsPull:
    ...
    def _shim(self, pulls, settle=2.0):
        clock = _Clock()
        return types.SimpleNamespace(
            name="AMS", logger=_Logger_ams(), pull_settle_s=settle,
            pull_push_dwell_s=0.0, arrival_select=True,
            afc=types.SimpleNamespace(reactor=clock),
            _pull_seq_now=lambda: pulls["n"],
...
class TestNoSwitchMeansNoPullToWaitFor:
    ...
    def test_it_does_not_wait_when_the_select_is_off(self):
        clock = _Clock()
        shim = types.SimpleNamespace(
            name="AMS", logger=_Logger_ams(), pull_settle_s=6.0,
    ...
    def test_it_still_waits_when_the_select_is_on(self):
        clock = _Clock()
        shim = types.SimpleNamespace(
            name="AMS", logger=_Logger_ams(), pull_settle_s=1.0,
            pull_push_dwell_s=0.0, arrival_select=True,
            afc=types.SimpleNamespace(reactor=clock),
            _pull_seq_now=lambda: 0, _assist_seq_now=lambda: 0)
        assert afcBambuAMS._wait_for_pull(shim, 0, 0) is False
...
class TestALateTagBeatsANoTagVerdict:
    ...
    def _u(self, info):
        ...
        u = types.SimpleNamespace(
            ...
            unit_slots=4, logger=_Logger_ams(), surfaced=[], finalized=[],
            _ACTIVE_STATES=afcBambuAMS._ACTIVE_STATES,
            afc=types.SimpleNamespace(
...
class TestPrepKeepsWhatAFCRestored:
    ...
    def _unit(self, lane, info):
        u = types.SimpleNamespace(
            ...
            lanes={lane.name: lane}, logger=_Logger_ams(),
...
class TestTheVariantComesBackFromTheBay:
    ...
    def _u(self):
        u = types.SimpleNamespace(name="BambuAMS_1", logger=_Logger_ams(),
                                  saved=[])
...
class TestAMS2WaitsForItsMeasurement:
    ...
    def _u(self, model, mseq0=None, mseq=None, ended=False):
        u = types.SimpleNamespace(
            name="BambuAMS_2", ams_model=model, logger=_Logger_ams(),
...
class TestAMeasurementOutlivesTheSession:
    ...
    def _u(self, lane):
        u = types.SimpleNamespace(
            name="BambuAMS_1", logger=_Logger_ams(), saves=[],
...
# ── ams_flows ─────────────────────────────────────────────────────────────────
#
# was tests/test_AFC_bambu_ams_flows.py
