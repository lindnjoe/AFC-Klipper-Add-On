...
def _sync_honouring_on_done(*a, **kw):
    """Stand-in for sync_rfid_to_spoolman that keeps its promise.

    The real one calls on_done on every path -- that is how a caller learns
    the lane actually carries the spool, now that the Spoolman round trips run
    on moonraker's writer thread instead of the reactor. A patch that swallowed
    the callback would report the notification as never sent, which is a fault
    in the fake rather than in the code.
    """
    done = kw.get("on_done")
    if done is not None:
        done()
...
class _LookupPrinter:
    ...
    def load_object(self, config, name, default=None):
        """Real printers always have this, and CONSTRUCT the object when its
        section has not been reached yet. That is how these modules take AFC's
        logger at __init__ (as AFC_lane and AFC_buffer do), so a fake without
        it fails during construction rather than at the call under test."""
        obj = self.lookup_object(name, None)
        if obj is not None:
            return obj
        return types.SimpleNamespace(logger=MockLogger())
...
def _build(values=None, afc=None, objects=None, webhooks=None):
    ...
    # No logging.getLogger to patch any more: the module takes AFC's logger
    # from load_object in __init__, which the fake printer supplies.
...
class TestAFCU1RFIDPatchScannerRfidUpdate:
    ...
    def test_patches_callback_and_suppresses_scanner_channel(self):
        ...
        # The firmware in the field: (channel, info, is_clear), positional.
        # Pinned so the arity-agnostic forwarding tested below cannot later be
        # narrowed to a newer API only.
        ...
        patched(3, {"y": 2})
        assert original.last == ((3, {"y": 2}), {})
        before = original.last
        patched(0, {"x": 1})
        assert original.last == before       # scanner channel still suppressed

    def test_survives_a_firmware_that_calls_it_differently(self):
        """The U1's own code calls this, so its arity is not ours to assume.

        A Snapmaker update that adds an argument -- or passes one by keyword --
        must not raise from inside our wrapper: an exception on filament_detect's
        notify path shuts Klipper down. Forward whatever arrives.
        """
        original = _Recorder()
        ptc = _FakePTC(cb=original)
        fd = _FakeFD(cb_list=[original])
        ...
        # An extra positional argument a future firmware might add.
        patched(3, {"y": 2}, True, "source")
        assert original.last == ((3, {"y": 2}, True, "source"), {})
        # The same call by keyword.
        patched(3, {"y": 2}, is_clear=True, official=False)
        assert original.last == ((3, {"y": 2}), {"is_clear": True,
                                                 "official": False})
        # Channel by keyword only -- suppression still has to work, so the
        # original must not be reached and its last call must be unchanged.
        before = original.last
        patched(channel=0, info={"x": 1})
        assert original.last == before
        # Nothing recognisable as a channel: pass it on rather than guess.
        patched("not-a-channel", {"z": 3})
        assert original.last == (("not-a-channel", {"z": 3}), {})
    ...
    def test_full_lane_load(self):
        ...
        with patch("extras.AFC_U1_rfid.apply_filament_defaults") as apply_def, \
                ...
                patch("extras.AFC_U1_rfid.sync_rfid_to_spoolman",
                      side_effect=_sync_honouring_on_done) as sync, \
    ...
    def test_colorless_tag_desc_has_no_color_label(self):
        ...
        with patch("extras.AFC_U1_rfid.apply_filament_defaults"), \
                patch("extras.AFC_U1_rfid.get_auto_spoolman_create",
                      return_value=False), \
                patch("extras.AFC_U1_rfid.sync_rfid_to_spoolman",
                      side_effect=_sync_honouring_on_done), \
...
class TestAFCU1RFIDNotifyScan:
    ...
    def test_scanner_prompt_includes_temps_and_spool_id(self):
        ...
        # Temps come off the lane, which set_spoolID populated from its own
        # fetch -- no second Spoolman GET on the reactor. Name/brand/material
        # stay the tag's here because the lane carries none.
        lane.extruder_temp = 210
        lane.bed_temp = 60
        # The stored record wins over the tag -- as it did through
        # enrich_from_spool -- so a realistic lane carries Spoolman's material,
    ...
    def test_enrichment_overlays_spool_record(self):
        """The stored record now comes off the LANE, not a fresh Spoolman GET.

        enrich_from_spool was a second blocking Spoolman call on the reactor,
        on every scan. set_spoolID already fetches the spool and writes
        name/vendor/material/temps onto the lane, and we only get here through
        its on_done -- so the lane is the newest read in the system AND the one
        that was applied, which means the popup cannot disagree with the lane
        it describes.
        """
        afc = MockAFC()
        reader = self._reader(afc=afc)
