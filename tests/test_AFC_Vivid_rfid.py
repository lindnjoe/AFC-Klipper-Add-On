...
class _Printer:
    ...
    def load_object(self, config, name, default=None):
        """Real printers always have this, and the module takes AFC's logger
        with it at construction -- as AFC_lane and AFC_buffer do."""
        obj = self._objects.get(name)
        if obj is not None:
            return obj
        return types.SimpleNamespace(logger=logging.getLogger("test.vivid_rfid"))
...
class _AFCStub:
    def __init__(self, lanes=None, spoolman=None, homing_enabled=True):
        self.lanes = lanes or {}
...
def test_stage_own_parked_tag_does_not_move_the_sibling(monkeypatch):
    # A shared antenna is shared in both directions: this slot's own spool is
    # stationary too, so ITS tag can be the one on the coil. Seen on a
    # BoxTurtle with the same logic -- the active lane's own tag read as
    # parked and the sibling was moved for nothing.
    r0 = _FakeReader("reader0", [0, 1])
    obj, printer = _make({"lane_slot_map": "lane0:0, lane1:1"}, readers=[r0])
    _fire_ready(obj, printer)
    obj.afc = _AFCStub(lanes={"lane0": _Lane("lane0"), "lane1": _Lane("lane1")})
    obj._slot_uid[0] = "CAFE"                     # slot 0's own tag
    calls = []
    monkeypatch.setattr(obj, "_retract_sibling", lambda p: calls.append(p))
    monkeypatch.setattr(obj, "_parked_tag", lambda slot: "cafe")
    obj._stage_read_begin(_Lane("lane0"))
    assert calls == []                            # sibling untouched
    assert obj._probe.get("baseline") is None     # and nothing excluded


def test_stage_an_unknown_parked_tag_is_still_cleared(monkeypatch):
    # The first-encounter case the clear exists for: no history to attribute
    # it by, so it stays a collision.
    r0 = _FakeReader("reader0", [0, 1])
    obj, printer = _make({"lane_slot_map": "lane0:0, lane1:1"}, readers=[r0])
    _fire_ready(obj, printer)
    obj.afc = _AFCStub(lanes={"lane0": _Lane("lane0"), "lane1": _Lane("lane1")})
    calls = []
    monkeypatch.setattr(obj, "_retract_sibling", lambda p: calls.append(p))
    monkeypatch.setattr(obj, "_parked_tag", lambda slot: "DEADBEEF")
    obj._stage_read_begin(_Lane("lane0"))
    assert len(calls) == 1
...
# ── Sibling clear / restore: the load switch is the reference ────────────────

class _MovingUnit:
    """The unit's move_to_load helper -- AFC's own, the one every load and
    unload uses. It stops ON the switch and leaves it made."""

    def move_to_load(self, lane, dist, dir, use_homing=True, speed_mode=None):
        return lane.move_to(dist * dir, speed_mode, endstop=lane.load_es,
                            use_homing=use_homing)


class _MovingLane:
    """A sibling whose LOAD switch tracks where the filament actually is, so a
    retract that must stop at the switch and a restore that must home back
    onto it can be tested rather than only their arithmetic.

    ``slip`` is the fraction of each FEED the filament does not travel -- gears
    skidding on a spool that has not started turning. It is one-directional,
    which is what makes a blind give-back walk the tip out of the lane.
    """

    def __init__(self, name="lane1", pos=500.0, load_at=0.0, slip=0.0):
        self.name = name
        self.prep_state = True
    ...
    def move_to(self, distance, speed_mode, endstop=None, assist_active=None,
                use_homing=False):
        self.moves.append(distance)
        self.homing_moves.append((distance, endstop, use_homing))
        self.assists.append(assist_active)
        if use_homing and endstop is not None:
            # Stops ON the switch and leaves it made -- triggered=True with
            # check_trigger=True, the same contract eject_lane relies on.
            edge = self.load_at + 0.5
            self.pos = (min(edge, self.pos + distance) if distance > 0
                        else max(edge, self.pos + distance))
...
def _sibling_rig(monkeypatch, sib, dist=75.0):
    """A Vivid unit wired so _retract_sibling resolves `sib` as slot 0's
    sibling and nothing else gets in the way."""
    r0 = _FakeReader("reader0", [0, 1])
    obj, printer = _make({"lane_slot_map": "lane0:0, lane1:1"}, readers=[r0])
    _fire_ready(obj, printer)
    obj.afc = _AFCStub(lanes={"lane0": _Lane("lane0"), "lane1": sib})
    obj.auto_tag_adjust = True
    obj.auto_tag_adjust_dist = dist
    monkeypatch.setattr(obj, "_afc_is_printing", lambda: False)
    obj.afc.homing_enabled = True
    return obj


class TestVividSiblingMoves:
    def test_a_seated_sibling_is_retracted_the_full_distance(self, monkeypatch):
        sib = _MovingLane()
        obj = _sibling_rig(monkeypatch, sib)
        p = {"slot": 0}
        obj._retract_sibling(p)
        assert sum(sib.moves) == pytest.approx(-75.0)
        assert p["sib_dist"] == pytest.approx(75.0)

    def test_a_sibling_off_its_load_switch_is_never_moved(self, monkeypatch):
        # prep_state alone is the operator's hand touching the switch, not a
        # seated tip. Retracting on that walks the lane out of itself.
        sib = _MovingLane(pos=-5.0)          # prep true, load false
        obj = _sibling_rig(monkeypatch, sib)
        obj._retract_sibling({"slot": 0})
        assert sib.moves == []

    def test_the_retract_stops_at_the_load_switch(self, monkeypatch):
        # Only 25mm of filament past the switch: the 75mm ask must not be a
        # commitment, or the rest comes out of the lane.
        sib = _MovingLane(pos=25.0)
        obj = _sibling_rig(monkeypatch, sib)
        p = {"slot": 0}
        obj._retract_sibling(p)
        assert p["sib_dist"] < 75.0
        assert sib.pos >= -sib.short_move_dis      # at most one step past

    def test_the_restore_is_one_homing_move_onto_the_load_switch(
            self, monkeypatch):
        # AFC's own move, the same one a load and an unload make: the MCU
        # stops it on the endstop, so slip in the give-back cannot leave the
        # tip somewhere a count merely believes in.
        sib = _MovingLane(pos=35.0, load_at=10.0, slip=0.9)
        obj = _sibling_rig(monkeypatch, sib)
        p = {"slot": 0}
        obj._retract_sibling(p)
        assert not sib.raw_load_state              # retract ended off it
        sib.homing_moves.clear()
        obj._restore_sibling(p)
        homing = [m for m in sib.homing_moves if m[2]]
        assert len(homing) == 1                    # one move, not a creep loop
        assert homing[0][0] > 0 and homing[0][1] == "load"
        assert sib.raw_load_state                  # and it landed on the switch

    def test_without_homing_it_falls_back_to_the_stepped_creep(
            self, monkeypatch):
        # No endstop to stop on, so the old shape stands: give back what was
        # taken, then step forward while watching the switch.
        sib = _MovingLane(pos=35.0, load_at=10.0, slip=0.9)
        obj = _sibling_rig(monkeypatch, sib)
        obj.afc.homing_enabled = False
        p = {"slot": 0}
        obj._retract_sibling(p)
        sib.moves.clear()
        obj._restore_sibling(p)
        assert not any(m[2] for m in sib.homing_moves)   # never homed
        assert sib.raw_load_state
        assert len(sib.moves) > 1                        # crept

    def test_a_sibling_that_will_not_come_back_is_said_out_loud(self, monkeypatch):
        sib = _MovingLane(pos=100.0)
        obj = _sibling_rig(monkeypatch, sib)
        p = {"slot": 0}
        obj._retract_sibling(p)
        said = []
        obj.logger = types.SimpleNamespace(
            ...
            error=lambda m, **k: said.append(m))
        sib.slip = 1.0                             # feeds achieve nothing
        sib.pos = -50.0                            # and it is off the switch
        sib.load_at = 1e9                          # nothing can reach it
        obj._restore_sibling(p)
        assert any("NOT back on its load switch" in m for m in said)
