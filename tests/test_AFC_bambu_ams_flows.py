...
class _Bridge:
    ...
    def __init__(self, uids=(), finish=(0, False, ""), fault=None):
        ...
        if fault is not None:
            self.last_fault = lambda unit=None: self._fault
...
class TestAnnounceUnit:
    ...
    def test_an_unclaimed_pool_spare_defers_instead_of_registering_at_zero(self):
        """A fabricated spare has no unit_uid, so the UID guard never fired and
        it announced at the DEFAULT index 0. Eight unclaimed AMS HT spares then
        pushed htunit unit=0 on=1, flagging index 0 as an HT; the firmware
        addressed it as 0x80, the boxed AMS there never answered, so the chain
        stayed empty and nothing could ever resolve an index. Self-sustaining,
        and it survived every restart."""
        b = _Bridge()
        u = _unit(b, pool=True, unit_uid="", _id_resolved=False)
        u._send_ht_flag = lambda br: b.sent.append({"cmd": "htunit"})
        u._send_mc_addr = lambda br: b.sent.append({"cmd": "mcaddr"})
        afcBambuAMS._announce_unit(u)
        assert b.sent == [], f"spare registered at a guessed index: {b.sent}"
        assert u._announce_deferred is True

    def test_a_claimed_pool_unit_announces_normally(self):
        """Claiming clears self.pool and resolves the index, and the announce
        held above has to go out then -- deferring forever would be the same
        chain stall by another route."""
        b = _Bridge()
        u = _unit(b, ams_index=1, pool=False, unit_uid="ABC", _id_resolved=True)
        sent = []
        u._send_ht_flag = lambda br: sent.append("ht")
        u._send_mc_addr = lambda br: sent.append("mcaddr")
        afcBambuAMS._announce_unit(u)
        assert {"cmd": "units", "n": 2} in b.sent
        assert sent == ["ht", "mcaddr"]
...
class TestOnBridgeReconnect:
    ...
    def test_the_settle_re_asserts_the_loaded_lane(self):
        # A reconnect means the Pico rebooted (reflash, power-cycle) while
        # Klipper stayed up, so the AMS forgot which tray is threaded and
        # the follower went dead -- on hardware the loaded lane needed a
        # manual SET_LANE_LOADED. The announce settle now schedules the same
        # restore klippy:ready runs, so the follower comes back on its own.
        u = self._u()
        restored = []
        u._startup_restore_loaded = lambda: restored.append(True)
        afcBambuAMS._announce_after_settle(u)
        # Deferred behind a short settle so the fresh slot frame lands first.
        assert restored == []
        cb = u.afc.reactor.callbacks[-1][0]
        cb(u.afc.reactor.monotonic())
...
class TestFpsBufferValue:
    ...
    def test_the_firmware_seed_is_not_published_as_a_reading(self):
        # Printer 2, 2026-09-22: the AMS answered nothing all boot, and the
        # firmware's seeded buff of 100 read as FULLY COMPRESSED -- the most
        # load-like value on the scale. The gate believed it and announced
        # filament at the gears off a value no wire had ever carried.
        # buffn is the firmware's own count of writes to the field.
        assert afcBambuAMS.fps_buffer_value(
            self._u({"buff": 100, "buffn": 0, "buffraw": -1})) is None

    def test_a_real_reading_is_published_however_low_the_count(self):
        # One write is a reading. Only zero is the seed.
        assert afcBambuAMS.fps_buffer_value(
            self._u({"buff": 100, "buffn": 1})) == 1.0

    def test_firmware_too_old_to_count_writes_keeps_its_old_behaviour(self):
        # A missing key is "cannot tell", not zero. Reading it as zero would
        # blind the buffer on every bridge predating the counter.
        assert afcBambuAMS.fps_buffer_value(self._u({"buff": 60})) == 0.6
...
class TestFinalizeScanIsTheNoTagOutcome:
    ...
    def _u(self, *, present=True, material="", cap=None,
...
class TestPrepClaimedLanes:
    """A pool unit brought online by claim() never ran AFC's PREP lane-test for
    its lanes (PREP ran at boot, when it was an inert pool slot), so the tag
    records of already-loaded bays never reached the lanes. _prep_claimed_lanes
    is the claim-time equivalent of PREP's data half: blank lane gets the tag,
    a lane AFC restored is deferred (_afc_owned), untagged bays are left alone."""

    def _u(self, slots):
        u = _unit(_Bridge())
        u._slots = slots
        u._slot_map = {f"lane{i}": i for i in range(len(slots))}
        u._afc_owned = set()
        u.lanes = {}
        for i in range(len(slots)):
            u.lanes[f"lane{i}"] = types.SimpleNamespace(
                name=f"lane{i}", material="", spool_id=None, weight=0)
        surfaced = []
        u._surface_slot_info = lambda lane, info: surfaced.append(lane.name)
        return u, surfaced

    def test_a_tagged_present_bay_surfaces_onto_a_blank_lane(self):
        u, surfaced = self._u([{"present": True, "material": "PLA Glow",
                                "rfid_uid": "fa0950a9"}])
        afcBambuAMS._prep_claimed_lanes(u)
        assert surfaced == ["lane0"]                 # the tag reached the lane
        assert 0 not in u._afc_owned

    def test_a_uid_only_present_bay_still_surfaces(self):
        # A foreign tag with a UID but no decoded profile is enough to match on.
        u, surfaced = self._u([{"present": True, "material": None,
                                "rfid_uid": "c0ffee01"}])
        afcBambuAMS._prep_claimed_lanes(u)
        assert surfaced == ["lane0"]

    def test_a_lane_afc_restored_is_deferred_not_overwritten(self):
        u, surfaced = self._u([{"present": True, "material": "PLA Glow",
                                "rfid_uid": "fa0950a9"}])
        u.lanes["lane0"].material = "PETG"           # restored from vars
        afcBambuAMS._prep_claimed_lanes(u)
        assert surfaced == []                        # AFC's data is kept
        assert 0 in u._afc_owned

    def test_a_spoolman_linked_lane_is_deferred(self):
        u, surfaced = self._u([{"present": True, "material": "PLA Glow",
                                "rfid_uid": "fa0950a9"}])
    ...
    def test_an_untagged_present_bay_is_left_for_the_defaults_pass(self):
        # No material AND no uid -> _restore_untagged_defaults handles it, not us.
        u, surfaced = self._u([{"present": True, "material": None,
                                "rfid_uid": None}])
        afcBambuAMS._prep_claimed_lanes(u)
        assert surfaced == []
    ...
    def test_an_empty_bay_is_ignored(self):
        u, surfaced = self._u([{"present": False, "material": "PLA Glow",
                                "rfid_uid": "fa0950a9"}])
        afcBambuAMS._prep_claimed_lanes(u)
        assert surfaced == []

    def test_mixed_bays_each_take_the_right_path(self):
        u, surfaced = self._u([
            {"present": True, "material": None, "rfid_uid": None},       # 0 empty-tag
            {"present": True, "material": "PLA Glow", "rfid_uid": "a1"},  # 1 tagged
            {"present": True, "material": "ABS", "rfid_uid": "b2"},       # 2 tagged
            {"present": False, "material": "X", "rfid_uid": "c3"},        # 3 absent
        ])
        u.lanes["lane2"].spool_id = 7                # 2 already linked
        afcBambuAMS._prep_claimed_lanes(u)
        assert surfaced == ["lane1"]                 # only the blank tagged one
        assert u._afc_owned == {2}


class TestPrintClaimedPrep:
    """PREP skips pool units and a spare claims its AMS after PREP, so a
    live-claimed unit never printed its PREP banner, lane lines or logo. The
    claim path prints that block itself once the bays have reported."""

    def _u(self, bridge):
        u = _unit(bridge, name="Bambu_AMS_1", type="BambuAMS",
                  logo="LOGO", logo_error="LOGO_ERR")
        u.logger.raw = lambda m: u.logger.messages.append(("raw", m))
        u._unit_online = lambda latest: True
        u._slots = [{"present": True, "material": "PLA Basic"},
                    {"present": False}]
        u._slot_map = {"lane8": 1, "lane7": 0}
        u.lanes = {n: types.SimpleNamespace(name=n, material="",
                                            tool_loaded=False, map=m,
                                            map_to_string=lambda m=m: m)
                   for n, m in (("lane7", "T7"), ("lane8", "NONE"))}
        return u

    def test_banner_lanes_in_bay_order_then_logo(self):
        bridge = _Bridge()
        bridge.latest_status = lambda: {}
        u = self._u(bridge)
        afcBambuAMS._print_claimed_prep(u)
        msgs = [m for _k, m in u.logger.messages]
        assert msgs[0] == "BambuAMS Bambu_AMS_1 Prepping lanes"
        assert msgs[1].startswith("lane7 tool cmd: T7 ")
        assert "LOCKED AND LOADED" in msgs[1] and "PLA Basic" in msgs[1]
        assert msgs[2] == "lane8 tool cmd: NONE EMPTY READY FOR SPOOL"
        assert u.logger.messages[-1] == ("raw", "LOGO")

    def test_no_bridge_prints_the_error_logo(self):
        u = self._u(None)
        afcBambuAMS._print_claimed_prep(u)
        assert u.logger.messages[-1] == ("raw", "LOGO_ERR")
...
class TestGcmdIntAcceptsHex:
    ...
    get_int is decimal-only, so AFC_BAMBU_HEATER_START ADDR=0x0700 -- written the
    way the protocol writes it -- was rejected as unparsable."""
    ...
    class _G:
        ...
        def get_commandline(self):
            return "AFC_BAMBU_HEATER_START ADDR=?"
    ...
    def test_garbage_is_an_error_not_a_default(self):
        # Silently falling back to the default would send the command somewhere
...
class TestStartupLeavesTheLaneAlone:
    """
    Bambu units behave like OpenAMS and ACE at startup: the lane is whatever
    AFC restored, and nothing in this module second-guesses it.

    There used to be a boot pass that filled a present-but-untagged bay with
    AFC defaults, so it showed PLA/1000 g instead of blank. It could not be
    made safe. Deciding anything at boot means deciding whether the lane is
    empty, and at boot that is unknowable -- a lane's Spoolman record arrives
    from an async fetch that lands seconds later, so "empty" and "not restored
    yet" are identical. It guessed wrong and overwrote lane15's binding to
    spool 159 on every restart. Three attempts to guess more carefully each
    tested green and each still lost it; the last guarded one caller while the
    toolhead-hold path went on overwriting.

    A bay with no tag now gets its profile from a real scan -- AFC_BAMBU_SCAN,
    or the next insert -- which is the only moment anything here learns
    something about the spool.
    """
    ...
    def test_a_caller_that_did_not_scan_changes_nothing(self):
        u = self._u([{"present": True, "material": None}])
        afcBambuAMS._finalize_scan(u, 0, scanned=False)
        assert u.lanes["lane0"].material is None, (
            "startup must not put defaults on a lane")
        assert u.afc.saves == 0, "and must not write anything"

    def test_a_bound_lane_is_untouched_even_when_the_bay_has_no_tag(self):
        # the case that cost spool 159
        u = self._u([{"present": True, "material": None}])
        u.lanes["lane0"].spool_id = 159
        u.lanes["lane0"].material = "PLA Matte"
        afcBambuAMS._finalize_scan(u, 0, scanned=False)
        assert u.lanes["lane0"].spool_id == 159
        assert u.lanes["lane0"].material == "PLA Matte"
        assert u.afc.saves == 0

    def test_a_real_scan_that_read_nothing_still_applies_defaults(self):
        # unchanged: that caller HAS looked at the bay and found no tag
        u = self._u([{"present": True, "material": None}])
        afcBambuAMS._finalize_scan(u, 0)
        assert u.lanes["lane0"].material == "Unknown"
        assert u.afc.saves == 1
...
# ── the toolhead retract has to be visible ───────────────────────────────────

class TestTheToolheadRetractIsLogged:
    """move_e_pos takes a log_string and never uses it.

    The parameter is dead upstream and all 17 callers hand it a description
    that reaches no log, so when a retract stalled with the toolhead sensor
    still triggered there was no way to tell whether this move had gone out at
    all -- "the AMS was pulling against clamped gears" and "the AMS jammed on
    its own" read identically. We log it ourselves at the call site.
    """

    def test_the_call_site_logs_before_issuing_the_move(self):
        import inspect
        src = inspect.getsource(afcBambuAMS.unit_unload_lane)
        assert "toolhead retract issued" in src, \
            "the retract goes out with nothing in the log again"
        # And it must say the distance, the speed and where E started, or the
        # line cannot answer the question it exists for.
        for token in ("tool_stn_unload", "tool_unload_speed", "E="):
            assert token in src, token

    def test_it_says_issued_not_finished(self):
        """wait_tool=False is deliberate -- the move overlaps the AMS reel --
        so the line must not read as proof the filament moved."""
        import inspect
        src = inspect.getsource(afcBambuAMS.unit_unload_lane)
        assert "queued, overlaps the AMS reel" in src
        assert "wait_tool=False" in src
