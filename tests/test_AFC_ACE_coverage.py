...
class TestCreateSerialLogger:
    def test_returns_none_without_a_log_file(self):
        # No log_file in start_args -> nowhere to put a dedicated serial log,
        # so it declines rather than inventing a path. (The size-capped
        # handler itself is covered in test_AFC_ACE_serial_log.py.) Clear the
        # process-global logger first so a handler left by another test does
        # not make the early "already set up" return hand back a logger.
        import logging
