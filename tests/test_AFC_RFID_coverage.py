...
class _FakeMoonraker:
    ...
    def get_spool(self, id, callback):
        # Upstream's signature since PR #844: fire-and-forget, takes a
        # callback, returns None. Kept exact on purpose -- a fake with the old
        # blocking signature is what let the real regression through.
        callback(self.spools.get(id))


def _serves_spools(mr, other):
    """Responder that answers get_spool's read from ``mr.spools`` and defers
    everything else to ``other``. get_spool goes through the proxy now, so a
    test that stubs the proxy has to answer that read too."""
    def responder(m, p, body=None, print_error=True):
        if m == "GET" and p.startswith("/v1/spool/"):
            return mr.spools.get(int(p.rsplit("/", 1)[1]))
        return other(m, p, body)
    return responder
...
def _client(responder=None):
    ...
    def proxy(method, path, body=None, print_error=True):
        ...
        # get_spool reads through the proxy now, so serve it from the same
        # dict the tests populate rather than making each one stub it.
        if method == "GET" and path.startswith("/v1/spool/"):
            return mr.spools.get(int(path.rsplit("/", 1)[1]))
        return None
...
# ── the roll identity: tray_uid ───────────────────────────────────────────────
#
# A Bambu spool carries a tag on EACH flange. Their 4-byte chip UIDs differ, so
# the same reel presents two identities depending on which way round it went
# into the bay, and card_uids -- keyed on the chip UID -- made two Spoolman
# records out of one reel with its consumption split between them. Measured on
# 2026-09-20 by moving three reels between bays: every one read a different chip
# UID from its other face, and every one already had two records. The 16-byte
# tray_uid was IDENTICAL across both tags of a reel and different between reels,
# so it is the ROLL's identity where the chip UID is one sticker's.
#
# Bambu only: no other brand here is known to write one, and every other reader
# keys off the chip UID.

class TestTrayUidField:
    def test_creates_missing_and_caches(self):
        client, calls = _client(lambda m, p, b: [] if m == "GET" else {"ok": 1})
        ...
        assert len([c for c in calls if c[0] == "GET"]) == 1
        assert [c for c in calls if c[0] == "POST"] == [
            ("POST", "/v1/field/spool/tray_uid",
             {"name": "Tray UID", "field_type": "text"})]

    def test_skips_existing(self):
        client, calls = _client(
            lambda m, p, b: [{"key": "tray_uid"}] if m == "GET" else None)
        client._ensure_tray_uid_field()
        assert [c for c in calls if c[0] == "POST"] == []


class TestWriteTrayUid:
    def test_writes_the_field_and_keeps_the_others(self):
        mr = _FakeMoonraker()
        mr.spools[7] = {"id": 7, "extra": {"card_uids": '"AABBCCDD"'}}
        client, calls = _client(_serves_spools(
            mr, lambda m, p, b: [{"key": "tray_uid"}] if m == "GET" else {}))
        client.write_tray_uid(7, "CF34CF1D212F46B5BC8561E05EB644C8")
        patch = [c for c in calls if c[0] == "PATCH"][0]
        extra = patch[2]["extra"]
        # Lower-cased for storage, and card_uids is untouched beside it.
        assert extra["tray_uid"] == '"cf34cf1d212f46b5bc8561e05eb644c8"'
        assert extra["card_uids"] == '"AABBCCDD"'

    def test_nothing_to_write_is_a_no_op(self):
        client, calls = _client(lambda m, p, b: None)
        assert client.write_tray_uid(7, "") is None
        assert client.write_tray_uid(7, "0" * 32) is None   # unread field
        assert calls == []
...
class TestFindSpoolByTrayUid:
    @staticmethod
    def _spools(client, rows):
        client.search_spools = lambda: rows

    def test_finds_the_roll_whichever_face_was_read(self):
        client, _ = _client()
        # One reel, two records is the BROKEN state; here the record has been
        # taught both faces, which is the fixed one.
        self._spools(client, [
            {"id": 3, "extra": {"tray_uid": '"cf34cf1d"',
                                "card_uids": '"D34E4E39,13F56D32"'}},
            {"id": 4, "extra": {"tray_uid": '"4e3177c3"'}}])
        assert find_spool_by_tray_uid(client, "CF34CF1D")["id"] == 3
        assert find_spool_by_tray_uid(client, "4e3177c3")["id"] == 4

    def test_an_ambiguous_answer_is_no_answer(self):
        # Two records for one roll is exactly the state this exists to end.
        # Picking one would bind the consumption to a coin flip.
        client, _ = _client()
        self._spools(client, [{"id": 3, "extra": {"tray_uid": '"cf34cf1d"'}},
...
class TestPatchSpool:
    ...
    def test_extra_updates_merges_existing(self):
        ...
        client._spoolman_proxy = _serves_spools(
            client._mr, lambda m, p, b: calls.append((m, p, b)) or {"ok": 1})
...
class TestWriteSpoolMetadata:
    ...
    def test_uid_merged_into_card_uids(self):
        spools = {5: {"extra": {"card_uids": json.dumps("AABB")}}}

        def responder(m, p, b):
            if (m, p) == ("GET", "/v1/field/spool"):
                return []
            if m == "GET" and p.startswith("/v1/spool/"):
                return spools.get(int(p.rsplit("/", 1)[1]))
            return {"ok": 1}

        client, calls = _client(responder)
        client._mr.spools[5] = spools[5]
...
class TestGetSpool:
    def test_reads_through_the_spoolman_proxy(self):
        # NOT through AFC_moonraker.get_spool: PR #844 made that async
        # (get_spool(id, callback) -> None), and delegating to it returned
        # None for every caller here.
        client, calls = _client()
        client._mr.spools[3] = {"id": 3}
        assert client.get_spool(3) == {"id": 3}
        assert calls == [("GET", "/v1/spool/3", None)]

    def test_a_non_dict_answer_is_no_spool(self):
        client, _ = _client(lambda m, p, b: "not a spool")
...
class TestWriteFlowK:
    ...
    def test_writes_rounded_k_merged(self):
        ...
        client._spoolman_proxy = _serves_spools(
            client._mr,
            lambda m, p, b: calls.append((m, p, b)) or (
                [] if m == "GET" else {"ok": 1}))
...
def _sync(monkeypatch, slot_info, stub, *, allow_create=True, set_next=False,
    ...
    # The match point every reader goes through: the roll first, the chip UID
    # second, and the id of a duplicate record when the reel is in twice. The
    # stub answers with whatever spool the test set up, matched by the tag.
    monkeypatch.setattr(_rfidmod, "find_spool_by_uid", lambda c, u: stub._spool)
    monkeypatch.setattr(_rfidmod, "match_spool_for_tag",
                        lambda c, u, t="": (stub._spool, False, None))
    logger = _Logger()
...
class TestSyncRfidToSpoolman:
    ...
    def test_outer_exception_logs_error(self, monkeypatch):
        ...
        assert logger.messages == [
            ("info", "TEST: matched spool #300 by tag UID AABB"),
            # "apply", not "sync": the split can now say WHICH half failed --
            # the Spoolman round trips or assigning the lane. set_spoolID
            # raising is the second one.
            ("error", "TEST Spoolman apply failed for lane9: kaboom")]


    def test_a_reel_recorded_twice_is_named_where_it_is_actionable(
            self, monkeypatch):
        """
        The duplicate is reported at the bind, not in an audit command.

        A spool with a tag on each side gets two Spoolman records, one per
        chip UID, and its filament count splits between them by which way
        round it went into the bay -- three reels here were like that for
        months and nothing ever said so. The roll match picks the right
        record; this is what makes the wrong one visible, while the reel is
        in front of someone.
        """
        matched = {"id": 124, "remaining_weight": 800,
                   "filament": {"id": 88, "name": "Bambu PLA Matte",
                                "material": "PLA", "density": 1.24,
                                "diameter": 1.75, "color_hex": "757575"}}
        stub = _SyncStub(spool=matched)
        monkeypatch.setattr(_rfidmod, "SpoolmanClient", lambda mr: stub)
        monkeypatch.setattr(_rfidmod, "match_spool_for_tag",
                            lambda c, u, t="": (matched, True, 132))
        logger = _Logger()
        spool_ns = _ns(next_spool_info=None, next_spool_id=None,
                       set_spoolID=lambda lane, sid: None)
        afc = _ns(spoolman=object(), moonraker=object(), spool=spool_ns)
        lane = _ns(name="lane14", spool_id=None, send_lane_data=lambda: None)
        sync_rfid_to_spoolman(afc, lane, {
            "uid": "13F56D32", "tray_uid": "cf34cf1d", "material": "PLA",
            "color_hex": "757575", "diameter": 1.75}, logger, "TEST")
        warned = [m for lvl, m in logger.messages if lvl == "warning"]
        assert any("#132" in m and "#124" in m for m in warned), warned
        assert any("SAME PHYSICAL REEL" in m for m in warned), warned
