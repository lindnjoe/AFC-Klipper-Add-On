...
Out of scope (large hardware orchestration state machines exercised only
through their smaller sub-steps above, to avoid excessive magic mocking):
  _oams_load, _oams_unload, _oams_load_inner, _oams_unload_inner,
  _verify_engagement, _advance_tool_stn_to_nozzle, _oams_extrude,
  _wait_for_hub_settle, calibrate_td1 / TD-1 capture helpers.
"""
...
class TestSyncLanesFromHardware:
    ...
    def test_unmapped_lane_skipped(self):
        ...
        assert not hasattr(lane, "prep_state") or lane.prep_state != True  # noqa: E712
