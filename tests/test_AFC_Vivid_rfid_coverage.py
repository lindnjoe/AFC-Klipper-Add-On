...
class _MoveLane:
    """A sibling with a LOAD switch, because the retract is bounded by it and
    the restore homes back onto it. ``pos`` defaults far enough past the
    switch that a full retract never releases it, which is a properly staged
    lane; tests that want the badly-staged case turn it down."""

    def __init__(self, name, prep_state=True, tool_loaded=False, pos=500.0):
        self.name = name
...
class _Assist:
    NO = "NO"
    YES = "YES"      # retracts run the spooler so the reel stays tight
...
class TestVividRetractSibling:
    ...
    def test_noop_when_movedirection_none(self, monkeypatch):
        monkeypatch.setattr(vivid, "MoveDirection", None)
        r0 = _FakeReader("reader0", [0, 1])
        obj, printer = _make({"lane_slot_map": "lane0:0, lane1:1"},
                             readers=[r0])
        _ready(obj, printer)
    ...
    def test_retracts_idle_sibling_and_records(self, monkeypatch):
        ...
        p = {"slot": 0}
        obj._retract_sibling(p)
        # Stepped now, not one 75mm commitment: the distance is a MAXIMUM the
        # load switch can cut short, which is what stops a badly staged lane
        # being walked out of itself.
        assert sum(m[0] for m in sib.moves) == pytest.approx(75.0 * _MoveDir.NEG)
        assert all(m[1:] == (_Speed.SHORT, _Assist.YES, False)
                   for m in sib.moves)
        assert p["sib_lane"] is sib
        assert p["sib_dist"] == 75.0
        assert obj.logger.messages == [
            ("info", "ViViD RFID: retracting sibling lane1 up to 75mm to "
                     "clear the antenna before reading slot 0")]

    def test_an_unseated_sibling_is_never_retracted(self, monkeypatch):
        # prep_state is the operator's hand on a switch, not a seated tip.
        _patch_enums(monkeypatch)
        obj = self._obj()
        sib = _MoveLane("lane1", pos=-5.0)      # prep true, load false
        obj.afc = _AFCStub(lanes={"lane1": sib})
        monkeypatch.setattr(obj, "_afc_is_printing", lambda: False)
    ...
    def test_the_retract_stops_at_the_load_switch(self, monkeypatch):
        _patch_enums(monkeypatch)
        obj = self._obj()
        sib = _MoveLane("lane1", pos=25.0)      # only 25mm to give back
        obj.afc = _AFCStub(lanes={"lane1": sib})
        monkeypatch.setattr(obj, "_afc_is_printing", lambda: False)
        p = {"slot": 0}
        obj._retract_sibling(p)
...
class TestVividRestoreSibling:
    ...
    def test_restores_and_resets(self, monkeypatch):
        ...
        p = {"sib_lane": sib, "sib_dist": 75.0}
        obj._restore_sibling(p)
        # A properly staged sibling never releases its load switch, so there
        # is nothing to home against and the give-back stands alone.
        assert sib.moves == [(75.0 * _MoveDir.POS, _Speed.SHORT,
                              _Assist.NO, False)]
        assert p["sib_lane"] is None and p["sib_dist"] == 0.0
        assert obj.logger.messages == []

    def test_a_give_back_that_lands_short_creeps_onto_the_switch(
            self, monkeypatch):
        _patch_enums(monkeypatch)
        obj, printer = _make()
        # Further behind the switch than the give-back covers, so the creep
        # is the only thing that can put it back on.
        sib = _MoveLane("lane1", pos=-100.0)
        p = {"sib_lane": sib, "sib_dist": 75.0}
        obj._restore_sibling(p)
        assert sib.raw_load_state               # crept back on
        assert len(sib.moves) > 1
        assert any("back on its load switch" in m
                   for _lvl, m in obj.logger.messages)
