...
class TestHandleConnect:
    ...
    def test_cancel_command_missing_sets_cancel_cmd_none(self):
        """The cancel command lookup failure is caught and cancel_cmd is set to
        None before the (single-arg) logger.warning call is attempted. The
        AFC_logger.warning API only accepts one positional argument, so the
        two-arg call in this except block itself raises -- which propagates
        to the outer handler and is logged as a general init failure instead.
        This test documents that actual, currently-buggy behavior.
        """
        oams = _make_oams()
