...
class _AfcLogger:
    """A double with AFC's logger signature, not the stdlib's.

    AFC's takes a finished message plus its own named keywords and has no
    exception(). A fake that accepted printf args would let calls the real
    logger rejects pass unnoticed here, which is how logger.exception() lived
    in this module for as long as it did.
    """
...
class _Printer:
    ...
    def load_object(self, config, name, default=None):
        """Real printers always have this. The module takes AFC's logger with
        it at construction, as AFC_lane and AFC_buffer do -- so a fake without
        it fails at __init__, not at the call under test."""
        obj = self.lookup_object(name, None)
        if obj is not None:
            return obj
        return types.SimpleNamespace(logger=_AfcLogger("AFC"))
...
def _make_rfid(ace2_obj, opts=None):
    ...
    obj.logger = _AfcLogger("test.ace2_rfid")
    obj.ace2 = ace2_obj
    return obj
...
def test_cmd_scan_defaults_to_sole_scanner_lane(monkeypatch):
    ...
    # The result goes to the CONSOLE now, not to the command: by the time the
    # scan finishes the command has long returned. (_Reactor runs the callback
    # inline, so it has already happened here.)
    assert any("staged next spool" in r for r in obj.gcode.info)


def test_cmd_scan_returns_before_the_scan_runs():
    """The scan held the g-code queue for up to ten minutes.

    It always yielded the REACTOR -- _dwell is reactor.pause() -- so the
    printer stayed responsive, but nothing else could run a command, which
    reads as a freeze. The command now hands the scan to a reactor callback
    and returns.
    """
    deferred = []
    a = _Ace2()
    obj = _make_rfid(a, {"lane_slot_map": "scan_lane:0",
                         "scanner_lanes": "scan_lane"})
    obj.afc = types.SimpleNamespace(spool=types.SimpleNamespace(next_spool_id=7))
    obj.reactor = types.SimpleNamespace(
        monotonic=lambda: 0.0,
        register_callback=lambda cb, waketime=None: deferred.append(cb))
    ran = []
    monkeypatch_scan = lambda name, secs: (ran.append(name) or
                                           {"uid": "abcd", "filament": {}})
    obj.scan_lane = monkeypatch_scan

    obj.cmd_ACE_RFID_SCAN(_GCmd({}))
    assert ran == [], "the command ran the scan itself"
    assert len(deferred) == 1, "the scan was not handed to the reactor"
    deferred[0](0.0)
    assert ran == ["scan_lane"]


def test_a_second_scan_is_refused_while_one_is_running():
    deferred = []
    obj = _make_rfid(_Ace2(), {"lane_slot_map": "scan_lane:0",
                               "scanner_lanes": "scan_lane"})
    obj.reactor = types.SimpleNamespace(
        monotonic=lambda: 0.0,
        register_callback=lambda cb, waketime=None: deferred.append(cb))
...
def test_the_running_flag_clears_even_when_the_scan_raises():
    """Otherwise one reader glitch locks the command out for good."""
    deferred = []
    obj = _make_rfid(_Ace2(), {"lane_slot_map": "scan_lane:0",
                               "scanner_lanes": "scan_lane"})
    obj.afc = types.SimpleNamespace(spool=types.SimpleNamespace(next_spool_id=None))
    obj.reactor = types.SimpleNamespace(
        monotonic=lambda: 0.0,
        register_callback=lambda cb, waketime=None: deferred.append(cb))
    ...
    obj.scan_lane = _boom
    obj.cmd_ACE_RFID_SCAN(_GCmd({}))
    deferred[0](0.0)                              # must not propagate
    assert obj._scan_running is False
    assert any("error" in r for r in obj.gcode.info)
    obj.cmd_ACE_RFID_SCAN(_GCmd({}))              # and the command works again
