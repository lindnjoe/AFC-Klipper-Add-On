...
# ── stubs ────────────────────────────────────────────────────────────────────
class _Logger:
    """Recording logger with AFC's signature, storing (level, message).

    AFC's logger takes a FINISHED message plus its own named keywords, and it
    has no exception() -- so a fake that accepts printf args would let a call
    the real logger rejects pass here unnoticed. Matching the real signature
    ...
    def error(self, msg, traceback=None, stack_name=""):
        # Recorded as "exception" when a traceback came with it, so the tests
        # that used to assert on logger.exception still describe the same
        # event: an error path that carried a stack.
        self.messages.append(("exception" if traceback else "error", msg))
...
class TestReadTag:
    ...
    def test_ntag_without_decode(self, monkeypatch):
        class _MC:
            ...
            def read_ntag(self, n=128, start_page=0):
                if start_page:                        # the AFC block probe
                    return None
...
class TestSpoolDetails:
    @staticmethod
    def _wire(obj, monkeypatch, get_spool):
        """Point the module's SpoolmanClient at ``get_spool``.

        _spool_details reads through SpoolmanClient now, not through
        moonraker's own get_spool -- PR #844 made that async (it takes a
        callback and returns None), so the old direct call returned nothing
        for every caller. These tests are about the merging that follows the
        read, so the read itself is stubbed at that seam.
        """
        obj.afc = types.SimpleNamespace(moonraker=object())
        monkeypatch.setattr(
            rfid, "SpoolmanClient",
            lambda mr: types.SimpleNamespace(get_spool=get_spool))
    ...
    def test_base_when_spool_not_dict(self, monkeypatch):
        obj = _mk(_Ace2(), {})
        self._wire(obj, monkeypatch, lambda sid: None)
        d = obj._spool_details(5, _slot_info())
        assert d["name"] == ""
    ...
    def test_enriched_from_spoolman(self, monkeypatch):
        ...
        self._wire(obj, monkeypatch, lambda sid: spool)
        d = obj._spool_details(5, _slot_info())
...
class TestCmdRfidScan:
    """The scan runs BEHIND the command now.

    cmd_ACE_RFID_SCAN hands it to a reactor callback and returns, so the
    g-code queue is free while a reader is watched for up to ten minutes.
    These fakes run the callback inline, so the scan has already happened by
    the time the command returns here -- but its results go to the CONSOLE
    (gcode.respond_info), because in the real thing there is no command left
    to answer by then.
    """
    ...
    def test_error_swallowed(self):
        ...
        assert any("scan: error: glitch" in r for r in obj.gcode.info)
        assert obj._scan_running is False
    ...
    def test_no_tag(self):
        ...
        gcmd = _GCmd({"LANE": "scan1"})
        obj.cmd_ACE_RFID_SCAN(gcmd)
        assert any("no tag found on scan1" in r for r in obj.gcode.info)
    ...
    def test_success_reports_staged_spool(self):
        ...
        assert any("staged next spool from scan1, uid=beef type=PLA "
                   "(spool #42)" in r for r in obj.gcode.info)
