...
class _Printer:
    ...
    def load_object(self, config, name, default=None):
        """Real printers always have this, and CONSTRUCT the object if the
        section has not been reached yet -- which is how the module takes
        AFC's logger at __init__, the way AFC_lane and AFC_buffer do."""
        if name in self.objects:
            return self.objects[name]
...
class TestHandleReady:
    ...
    def test_ready_takes_afc_and_leaves_the_logger_alone(self):
        """The logger is settled in __init__ now, not swapped here.

        load_object CONSTRUCTS AFC when this section is reached first, so there
        is no window where lines reach klippy.log only. What ready still does
        is take self.afc -- which is this module's READY MARKER, read by its
        guards, and so deliberately still None until now.
        """
        cal, printer = _make()
        before = cal.logger
        afc = _make_afc()
    ...
    def test_the_logger_is_afcs_from_construction(self):
        import inspect
        src = inspect.getsource(AFC_autocal.__init__)
...
class TestPatchSetSpoolidEmit:
    """The wrapper must survive upstream's signature, not restate it.

    These tests used to stub set_spoolID as
    ``(self, cur_lane, SpoolID, save_vars=True)`` and assert against THAT --
    so when upstream added ``on_done`` the suite stayed green while the real
    call raised TypeError and shut Klipper down. Every stub here is now built
    from ``inspect.signature`` of the genuine function, so the contract under
    test is upstream's and drifting from it fails here first.
    """

    @staticmethod
    def _real_signature():
        """The genuine upstream set_spoolID signature, unpatched."""
        import importlib
        import inspect
        return inspect.signature(
            importlib.import_module("extras.AFC_spool").AFCSpool.set_spoolID)

    def test_accepts_every_upstream_parameter(self, monkeypatch):
        """The wrapper takes any call upstream itself would take."""
        import inspect
        from extras.AFC_spool import AFCSpool
        sig = self._real_signature()
        monkeypatch.setattr(AFCSpool, "_afc_autocal_spoolid_patched", False,
                            raising=False)
        cal, _ = _make()
        cal._patch_set_spoolid_emit()
        wrapped = inspect.signature(AFCSpool.set_spoolID)
        lane = _make_lane()
        # Bind a full call -- every parameter upstream declares, by keyword --
        # against the wrapper. This is the assertion the old tests lacked.
        sample = {"cur_lane": lane, "SpoolID": 7, "save_vars": False,
                  "on_done": lambda: None}
        kwargs = {n: sample[n] for n in sig.parameters
                  if n != "self" and n in sample}
        assert set(kwargs) == {n for n in sig.parameters if n != "self"}, (
            "upstream set_spoolID grew a parameter this test does not "
            "exercise: %s" % sorted(set(sig.parameters) - {"self"}
                                    - set(kwargs)))
        wrapped.bind(object(), **kwargs)          # raises if the arity drifted

    def test_emits_through_on_done_not_on_return(self, monkeypatch):
        """The event means 'the lane's data is applied', so it rides on_done.

        Upstream's fetch is async: set_spoolID returns before spool_id is set.
        Emitting on return made _do_spool_assigned read a lane that had no
        spool_id yet and bail, so staged spools were never calibrated.
        """
        from extras.AFC_spool import AFCSpool
        calls = {}

        def fake_set_spoolID(self, cur_lane, SpoolID, save_vars=True,
                             on_done=None):
            calls["args"] = (SpoolID, save_vars)
            calls["emitted_before_apply"] = getattr(
                self.printer, "sent_events", [])[:]
            cur_lane.spool_id = SpoolID          # what _apply_spool_data does
            if on_done is not None:
                on_done()

        monkeypatch.setattr(AFCSpool, "set_spoolID", fake_set_spoolID,
                            raising=False)
        monkeypatch.setattr(AFCSpool, "_afc_autocal_spoolid_patched", False,
                            raising=False)
        cal, _ = _make()
        ...
        assert calls["args"] == (7, False)
        assert calls["emitted_before_apply"] == []      # nothing emitted early
        assert fake_printer.sent_events == [("afc:spool_assigned", (lane,))]
        assert lane.spool_id == 7        # applied by the time we announced it

    def test_callers_on_done_still_runs_and_runs_first(self, monkeypatch):
        """cmd_SET_SPOOL_ID passes its own on_done; ours must not displace it."""
        from extras.AFC_spool import AFCSpool
        order = []
        ...
        fake_printer = _Printer()
        fake_printer.send_event = lambda name, *a: order.append(name)
        fake_spool = types.SimpleNamespace(printer=fake_printer)
        AFCSpool.set_spoolID(fake_spool, _make_lane(), 7,
                             on_done=lambda: order.append("caller"))
        assert order == ["caller", "afc:spool_assigned"]

    def test_event_survives_a_raising_caller_callback(self, monkeypatch):
        """A caller's on_done that blows up must not swallow the assignment."""
        from extras.AFC_spool import AFCSpool
        seen = []
        ...
        fake_printer = _Printer()
        fake_printer.send_event = lambda name, *a: seen.append(name)
        fake_spool = types.SimpleNamespace(printer=fake_printer)

        def boom():
            raise RuntimeError("caller callback failed")

        # The caller's exception still propagates -- we do not hide upstream
        # failures -- but our event has already been sent.
        try:
            AFCSpool.set_spoolID(fake_spool, _make_lane(), 7, on_done=boom)
        except RuntimeError:
            pass
        assert seen == ["afc:spool_assigned"]

    def test_falls_back_to_emit_on_return_without_on_done(self, monkeypatch):
        """If a later upstream drops on_done, emitting on return is right again."""
        from extras.AFC_spool import AFCSpool
    ...
    def test_bad_call_is_reported_by_upstream_not_the_wrapper(self, monkeypatch):
        """An argument error should name the real signature, not our wrapper."""
        from extras.AFC_spool import AFCSpool

        def upstream_set_spoolID(self, cur_lane, SpoolID, save_vars=True,
                                 on_done=None):
            return None

        monkeypatch.setattr(AFCSpool, "set_spoolID", upstream_set_spoolID,
                            raising=False)
        monkeypatch.setattr(AFCSpool, "_afc_autocal_spoolid_patched", False,
                            raising=False)
        cal, _ = _make()
        cal._patch_set_spoolid_emit()
        fake_spool = types.SimpleNamespace(printer=_Printer())
        with pytest.raises(TypeError) as exc:
            AFCSpool.set_spoolID(fake_spool, _make_lane(), 7, nonsense=1)
        assert "upstream_set_spoolID" in str(exc.value)
        assert "nonsense" in str(exc.value)

    def test_idempotent(self, monkeypatch):
        from extras.AFC_spool import AFCSpool
        sentinel = lambda self, cur_lane, SpoolID, save_vars=True, on_done=None: None
