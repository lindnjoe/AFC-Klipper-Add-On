...
The panel's job is to turn a mixed bag of vendor dryers into one view and one
set of controls, so these pin the parts where that translation can go wrong:
discovery and per-vendor descriptors, the status snapshot (which is the ONLY
thing the HTTP thread may read), colour normalisation across two different
vendor encodings, and above all request validation, the panel binds 0.0.0.0
and builds G-code, so an unvalidated field is a remote command shell.
"""
...
class FakeLogger:
    """AFC's logger signature, not the stdlib's."""
...
class FakePrinter:
    ...
    def load_object(self, config, name):
        # The panel takes AFC's logger at construction; load_object builds
        # AFC on demand, so it answers even with no [AFC] section.
        assert name == "AFC"
        return self._afc if self._afc is not None else types.SimpleNamespace(
            logger=self.logger)
...
class TestLiveClaimRefresh:
    """A Bambu pool slot changes what it IS when it claims a physical AMS live.
    _discover ran once at boot, so the card must (a) keep a heaterless-fabricated
    pool slot in the roster for a future claim, (b) hide it while unclaimed, and
    (c) refresh model/label/heater/ceiling from the unit once it is claimed --
    all without a Klipper restart."""

    def test_a_heaterless_pool_slot_is_kept_for_a_future_claim(self):
        u = FakeBambu("AMS_1", model="ams1", heater=False)
        u.pool = True
        panel, _ = _panel([u])
        assert [d["name"] for d in panel._units] == ["AMS_1"]

    def test_a_heaterless_NON_pool_unit_is_still_hidden(self):
        # The provisional-heater exception is only for pool slots; a genuinely
        # heaterless fixed unit is still hidden by default.
        panel, _ = _panel([FakeBambu("AMS_1", model="ams1", heater=False)])
        assert panel._units == []
    ...
    def test_an_unclaimed_pool_slot_is_hidden_from_the_page(self):
        ...
        panel, _ = _panel([u])
        panel._snapshot(0.0)
        assert panel.get_state()["units"] == []

    def test_a_live_claim_refreshes_model_label_heater_and_ceiling(self):
        # Fabricated ams1 (no heater), discovered as a pool slot and hidden.
        u = FakeBambu("AMS_1", model="ams1", heater=False)
        u.pool = True
        panel, _ = _panel([u])
        ...
        u.dry_max_temp = 65
        panel._snapshot(0.0)
        row = panel.get_state()["units"][0]
        assert row["model"] == "ams2"
        assert row["label"] == "AMS 2 Pro"
        assert row["max_temp"] == 65
        assert row["has_heater"] is True
...
class TestSnapshot:
    ...
    def test_the_ace_set_point_reaches_the_card(self):
        # Measured on a V1: dryer_status carries the set-point beside the
        # state word, so a running cycle has a target to show. The backend
        # used to hardcode None on the belief that it did not.
        panel, _ = _panel([FakeAce("Ace_1", status={
            "ace_connected": True, "ace_dryer": "drying",
            "ace_dryer_target": 55, "ace_temp": 51})])
        panel._snapshot(0.0)
        assert panel.get_state()["units"][0]["target"] == 55

    def test_a_v1_reports_its_cycle_length_in_minutes(self):
        # Measured: duration 480 beside 27679 seconds left. 480 cannot be
        # seconds -- there was more time left than the whole cycle.
        panel, _ = _panel([FakeAce("Ace_1", status={
            "ace_connected": True, "ace_dryer": "drying",
            "ace_dryer_remain": 27679, "ace_dryer_duration": 480})])
        panel._snapshot(0.0)
        u = panel.get_state()["units"][0]
        assert (u["remaining"], u["duration"]) == (27679, 28800)

    def test_an_ace_2_reports_the_same_field_in_seconds(self):
        # Measured on an ACE 2 asked for 120 minutes: duration 7200, which
        # cannot be minutes. Same field, same firmware family, other unit.
        class FakeACE2(FakeAce):
            PREFIX = "AFC_ACE2"

        panel, _ = _panel([FakeACE2("Ace2_1", status={
            "ace_connected": True, "ace_dryer": "keeping",
            "ace_dryer_remain": 7200, "ace_dryer_duration": 7200})])
        panel._snapshot(0.0)
        u = panel.get_state()["units"][0]
        assert (u["remaining"], u["duration"]) == (7200, 7200)

    def test_a_v1_near_the_end_is_not_mistaken_for_seconds(self):
        # The case that rules out deciding by magnitude: 480 minutes left
        # against 100 seconds remaining reads as seconds to any size test.
        panel, _ = _panel([FakeAce("Ace_1", status={
            "ace_connected": True, "ace_dryer": "drying",
            "ace_dryer_remain": 100, "ace_dryer_duration": 480})])
        panel._snapshot(0.0)
        assert panel.get_state()["units"][0]["duration"] == 28800

    def test_a_bambu_clock_reaches_the_card_in_the_same_units(self):
        panel, _ = _panel([FakeBambu("AMS_2", status={
            "bridge_online": True, "drying": True,
            "dry_remaining": 1800, "dry_minutes": 480})])
        panel._snapshot(0.0)
        u = panel.get_state()["units"][0]
        assert (u["remaining"], u["duration"]) == (1800, 28800)

    def test_a_unit_with_no_clock_reports_none_not_zero(self):
        # Zero length would draw a progress bar that is instantly full.
        panel, _ = _panel([FakeAce("Ace_1", status={
            "ace_connected": True, "ace_dryer": "drying",
            "ace_dryer_duration": 0})])
        panel._snapshot(0.0)
        u = panel.get_state()["units"][0]
        assert (u["remaining"], u["duration"]) == (None, None)

    def test_an_idle_ace_reports_no_clock_despite_its_zeros(self):
        # Measured on the wire: an idle ACE fills dryer_status with zeros.
        # Read literally that is a 0C set-point with 0 minutes left, which is
        # what a just-started cycle would show before the numbers catch up.
        panel, _ = _panel([FakeAce("Ace_1", status={
        ...
        panel._snapshot(0.0)
        u = panel.get_state()["units"][0]
        assert (u["target"], u["remaining"], u["duration"]) == (None, None, None)

    def test_an_idle_ace_has_no_target(self):
        panel, _ = _panel([FakeAce("Ace_1", status={
            "ace_connected": True, "ace_dryer": "stop"})])
        panel._snapshot(0.0)
        assert panel.get_state()["units"][0]["target"] is None
...
class TestGenericBackendListsEveryOtherUnit:
    ...
    def test_an_underscored_type_reads_as_its_camelcase_prefix(self):
        # Box_Turtle / Night_Owl / Quattro_Box default to an underscored type;
        # the label strips the underscore to match the real section prefix
        # (AFC_BoxTurtle), so the button reads "BoxTurtle", not "Box_Turtle".
        panel, _p = _panel(values={"show_heaterless": True},
                           afc_units={"Turtle_1": _OtherUnit(type_="Box_Turtle")})
        assert panel._units[0]["label"] == "BoxTurtle"
...
class _Printer:
    ...
    def lookup_objects(self, module=None):
        return [(n, o) for n, o in self._objects.items()
                if module is None or n.split(" ", 1)[0] == module]
...
class TestGenericBackendEnvironmentReadings:
    ...
    def test_the_case_of_the_section_name_does_not_have_to_match(self):
        # Measured on a real machine: [AFC_OpenAMS AMS_1] declares
        # `oams = OAMS1` while the sensor section is [temperature_sensor
        # oams1]. Matching those literally missed on capitalisation alone,
        # and the card showed nothing for a unit whose readings were live.
        snap = self._snap({"aht3x oams1": _Sensor(temperature=27.4,
                                                  humidity=38.0)},
                          oams_name="OAMS1")
        assert (snap["temperature"], snap["humidity"]) == (27.4, 38.0)
...
class _SensorAce(FakeAce):
    """A V1 ACE with an aht sensor wired beside it."""

    def __init__(self, objects, name="Ace_1", status=None):
        super().__init__(name, status=status)
        self.printer = _Printer(objects)


class TestAceFallsBackToAWiredSensor:
    """The V1 ACE has no humidity sensor -- its get_status carries temperature
    and the dryer state, and humidity arrived with the ACE 2. An owner who
    wires an aht10 beside a V1 had the reading on the printer and a dash on
    the card, because this backend only read what the unit itself reported."""

    def _snap(self, objects, status=None):
        return _AceBackend().snapshot(_SensorAce(objects, status=status),
                                      status or {})

    def test_a_v1_gets_its_humidity_from_the_sensor(self):
        snap = self._snap({"aht10 Ace_1": _Sensor(temperature=27.0,
                                                  humidity=44.0)},
                          {"ace_temp": 27, "ace_connected": True})
        assert snap["humidity"] == 44.0

    def test_the_units_own_reading_still_wins(self):
        # An ACE 2 reports humidity itself; a sensor nearby must not override
        # the unit's own number.
        snap = self._snap({"aht10 Ace_1": _Sensor(temperature=99.0,
                                                  humidity=11.0)},
                          {"ace_temp": 27, "ace_humidity": 39})
        assert snap["temperature"] == 27
        assert snap["humidity"] == 39
    ...
    def test_the_dryer_state_is_untouched_by_any_of_this(self):
        snap = self._snap({"aht10 Ace_1": _Sensor(temperature=27.0,
                                                  humidity=44.0)},
                          {"ace_dryer": "drying", "ace_connected": True})
        assert snap["drying"] is True
        assert snap["note"] == "drying"

    def test_an_unlisted_driver_chip_is_still_a_sensor(self):
        # Measured config: sensor_type aht3x, which no prefix list had. The
        # chip is not the point -- an object under the right name reporting a
        # reading IS the sensor.
        unit = _SensorAce({"aht3x Ace_1": _Sensor(temperature=27.0,
                                                  humidity=44.0)})
        snap = _AceBackend().snapshot(unit, {})
        assert snap["humidity"] == 44.0

    def test_a_sensor_named_something_else_is_found_when_declared(self):
        # Measured config: [temperature_sensor ace_temp] sensor_type: aht2x,
        # beside a unit called Ace_1. Nothing about "ace_temp" is guessable
        # from the unit's name, so the unit declares it.
        unit = _SensorAce({"aht2x ace_temp": _Sensor(temperature=27.0,
                                                     humidity=44.0)})
    ...
    def test_an_undeclared_sensor_under_another_name_is_not_guessed(self):
        # The same setup WITHOUT the declaration: nothing is found, and the
        # card says so with a dash rather than inventing a number.
        snap = self._snap({"aht2x ace_temp": _Sensor(temperature=27.0,
                                                     humidity=44.0)},
                          {"ace_temp": 51})
        assert snap["humidity"] is None
