...
try: from extras.AFC_utils import ERROR_STR
    ...
    error_str = f"Error when trying to import AFC_utils.ERROR_STR\n{traceback.format_exc()}"
...
def _ams_box_logo(title: str, n_slots: int, name: str) -> str:
    ...
    body = "\n".join(f"{L}  {r}" for L, r in zip("READY", rows))
...
def _ams_box_logo_error(title: str, n_slots: int, name: str) -> str:
    ...
    body = "\n".join(f"{L}  {r}" for L, r in zip("ERROR", rows))
...
class afcACE(afcUnit):
    ...
    def __init__(self, config: ConfigWrapper) -> None:
        ...
        # For a SINGLE ACE, /dev/serial/by-id/... is fine. For MULTIPLE ACEs on
        # one host, by-id is useless -- every ACE reports the same descriptor,
        # so they all collide on one by-id path. Pin each unit by its physical
        # USB PORT instead: use /dev/serial/by-path/... (run `ls -l
        # /dev/serial/by-path/` with the units plugged in). by-path is unique
        # per port AND udev re-points it to the unit's current ttyACM after each
        # re-enumeration, so it tracks the number-shuffle on its own.
        #
        # serial_port: auto -- autodetect this unit by its firmware id. Chained
        # ACEs each get their own USB device; the firmware self-reports a stable
        # per-unit id (get_info result.id) that counts up by cable order (first
        # unit = 1). With `auto`, we bind this section to the unit whose id
        # matches ace_index below, re-resolved on every reconnect so the ttyACM
        # shuffle never mis-binds. An explicit by-id/by-path pins a specific
        # port instead; omitting serial_port keeps today's single-unit behavior.
        self.serial_port = config.get("serial_port")
        # Which ACE in cable order this section is (= the firmware's get_info
        # id). Used only when serial_port: auto; harmless otherwise. Defaults to
        # 1 (the head/only unit), matching the firmware's 1-based numbering.
        self.ace_index = config.getint("ace_index", 1, minval=1)
    ...
    def handle_connect(self) -> None:
        ...
        # Hand the serial port (and its autodetect claim) back on every restart.
        # A FIRMWARE_RESTART rebuilds every config object in the SAME process, so
        # the module-level _ACE_CLAIMED_PORTS set and the open tty outlive the
        # old unit unless it lets go here. Without this, the rebuilt unit's
        # serial_port: auto probe finds its own port still claimed/held by the
        # dead instance and reports "no unit found" -- which is exactly why a
        # FIRMWARE_RESTART lost the ACE while a full `systemctl restart klipper`
        # (fresh process, empty globals) found it again. Klipper fires
        # klippy:disconnect on every teardown, so this covers both.
        self.printer.register_event_handler(
            "klippy:disconnect", self._handle_klippy_disconnect)

    def _handle_klippy_disconnect(self) -> None:
        """Release the ACE serial port and its autodetect claim on teardown.

        The claim lives in a process-global set that a soft restart does not
        reset, so the rebuilt unit cannot re-probe its own port until the old
        one hands it back. Safe with no live connection."""
        ace = getattr(self, "_ace", None)
        if ace is not None:
    ...
    def _create_serial_logger(self) -> Optional[Any]:
        """Dedicated logger for ACE serial comms -> AFC_ACE_serial.log.

        Size-capped in place so it can never fill the card: a
        RotatingFileHandler rolls the file over at _SERIAL_LOG_MAX_BYTES and
        keeps ONE backup (AFC_ACE_serial.log.1), so on-disk use tops out at
        ~2x that (the live file plus one rolled copy) and the previous chunk
        survives a rollover rather than being wiped. Records go through a queue
        so the serial thread never blocks on file I/O, and the listener is
        stopped at interpreter exit so it flushes and does not hang shutdown.
        """
        ...
        try:
            ...
            handler = logging.handlers.RotatingFileHandler(
                str(log_file), maxBytes=self._SERIAL_LOG_MAX_BYTES,
                backupCount=1)
            handler.setFormatter(logging.Formatter(
                "%(asctime)s %(message)s", datefmt="%H:%M:%S"))
            log_q: "queue.Queue" = queue.Queue(-1)
            ql = logging.handlers.QueueListener(
                log_q, handler, respect_handler_level=False)
            ql.start()
    ...
    def _on_hw_status_callback(self, response: dict) -> None:
        ...
        if action != self._current_action:
            self.logger.info(
                f'ACE {self.name}: {(self._current_action or "idle")} -> {(action or "idle")}')
    ...
    def _derive_action(result: dict) -> str:
        ...
        for slot in result.get("slots", []) or []:
            ...
            if st in busy:
                idx = slot.get("index")
                return f"{st}(slot {idx})" if idx is not None else st
    ...
    def get_status(self, eventtime: Optional[float] = None) -> dict:
        ...
        # dryer_status carries more than the state word: a live V1 reports
        # {'status': 'drying', 'target_temp': 55, 'duration': 480,
        #  'remain_time': 27679}. Only the word used to be published, so the
        # dryer card showed a running cycle with no set-point beside it.
        dryer = hw.get("dryer_status") or {}
        response["ace_dryer"] = dryer.get("status", "")
        response["ace_dryer_target"] = dryer.get("target_temp")
        response["ace_dryer_remain"] = dryer.get("remain_time")
        response["ace_dryer_duration"] = dryer.get("duration")
        ...
        for lane in self.lanes.values():
            ...
            if slot_ready and not slot_transient and prep_done and lane.status == AFCLaneState.NONE:
                if (lane.tool_loaded
                    ...
                    self._dispatch_load_runout(lane, eventtime, True)
            ...
            if (not slot_ready and not slot_transient
                ...
                if prev_ready:
                    eventtime = self.afc.reactor.monotonic()
                    self._dispatch_load_runout(lane, eventtime, False)

            if not slot_transient:
                self._prev_slot_states[lane.name] = slot_ready

    def _dispatch_load_runout(self, lane: AFCLane, eventtime: float,
                              load_state: bool) -> None:
        """Run a lane's load/runout handler with its failures made visible.

        handle_load_runout is where a runout turns into a full TOOL_UNLOAD, and
        this whole call chain runs inside the serial transport's frame dispatch
        (_handle_response), which catches every exception and drops it into a
        DEBUG line on the ACE serial log. That made a mid-unload failure
        invisible: no console message, no AFC.log entry, no AFC error state,
        and the lane left latched TOOL_UNLOADING with the position still saved.
        Catch it here instead, where AFC's logger and error handling are in
        reach, so the user sees what stopped the unload and the lane ends up in
        a state they can recover from.

        :param lane: Lane whose load switch changed.
        :param eventtime: Reactor event time of the change.
        :param load_state: True when filament is present at the load switch.
        """
        try:
            lane.handle_load_runout(eventtime, load_state)
        except Exception as e:
            self.logger.error(
                f"{lane.name}: {'load' if load_state else 'runout'} handling "
                f"failed: {e}",
                traceback=traceback.format_exc()
            )
            # Leave AFC in a state the user can act on: the unload aborted
            # part-way, so the toolhead still holds filament.
            try:
                self.afc.error.AFC_error(
                    f"Lane {lane.name} {'load' if load_state else 'runout'} "
                    f"handling failed: {e}\n"
                    "Filament may still be loaded in the toolhead, check the "
                    "toolhead before resuming.",
                    pause=self.afc.function.in_print()
                )
            except Exception:
                self.logger.debug("Unable to raise AFC error for the failed "
                                  "runout handler",
                                  traceback=traceback.format_exc())
    ...
    def _build_slot_map(self) -> dict:
        ...
        for lane_name, lane in self.lanes.items():
            ...
            if not (1 <= idx <= self.SLOTS_PER_UNIT):
                raise error(
                    (f"ACE {self.name}: lane '{lane_name}' has index {idx}, outside this unit's "
                        f"slots 1..{self.SLOTS_PER_UNIT}, fix the lane's index in the config."))
            slot = idx - 1
            if slot in owner:
                raise error(
                    (f"ACE {self.name}: lanes '{owner[slot]}' and '{lane_name}' both map to slot "
                        f"{slot} (index {idx}), each lane needs a unique index."))
       ...
       , a correctly configured unit never hits this (see the connect-time slot
        ...
        if slot is None:
            ...
            if lane_name not in warned:
                ...
                self.logger.warning(
                    f"ACE {self.name}: lane {lane_name!r} is not in this "
                    f"unit's slot map {sorted(self._slot_map)}, defaulting "
                    f"to slot 0; check the unit/lane configuration")
            return 0
        return slot
    ...
    def system_Test(self, cur_lane: AFCLane, delay: float, assignTcmd: bool,
        ...
        # virtual_tools: lane.map is a LIST now; formatting it as {:3} shut
        # klipper down at PREP ("unsupported format string passed to
        # list.__format__"). map_to_string() renders every mapped T(n).
    ...
    def cmd_ACE_BUFFER_STATE(self, gcmd: GCodeCommand) -> None:
        ...
        try:
            ...
            gcmd.respond_info(f"ACE_BUFFER_STATE: {e}")
            return
        ...
        for slot, s in enumerate(slot_sensors):
            ...
            lines.append(
                (f"  slot {slot} ({lane}): insert={int(s['insert'])} empty={int(s['empty'])} | "
                    f"buf_rst={int(s['buf_rst'])} buf_back={int(s['buf_back'])}"))
        lines.append(f"  shared buf_feed={int(state.get('buf_feed', False))}")
        gcmd.respond_info("\n".join(lines))
    ...
    def lane_unloading(self, lane: AFCLane) -> None:
        ...
        try:
            ...
            self.logger.warning(
                f"ACE: lane_unloading assist-stop error for {getattr(lane, 'name', '?')}: {e}")
    ...
    def cmd_ACE_FEED_INFO(self, gcmd: GCodeCommand) -> None:
        ...
        if not feeds:
            gcmd.respond_info(
                (f"ACE feed info: no data (unsupported on this firmware, or nothing fed yet). "
                    f"raw_fields={(result or {}).get('raw_fields')}"))
            return
        ...
        for i, fi in enumerate(feeds):
            ...
            lines.append(f"  slot {i}: {fi.get('steps', 0)} / {length} / {decoder} / {ratio:.3f}")
        gcmd.respond_info("\n".join(lines))
    ...
    def cmd_ACE_RFID_DUMP(self, gcmd: GCodeCommand) -> None:
        ...
        lines = [f"ACE RFID slot {slot}:", f"  parsed: {parsed}"]
        if raw is not None:
            lines.append(f"  raw protobuf fields (field#: value): {raw}")
        else:
    ...
    def cmd_ACE_TEMP_INFO(self, gcmd: GCodeCommand) -> None:
        ...
        gcmd.respond_info(
            "ACE temperatures:\n"
            f"  box1={temp.get('box1_temp', 'n/a')}  box2={temp.get('box2_temp', 'n/a')}\n"
            f"  ptc1={temp.get('ptc1_temp', 'n/a')}  ptc2={temp.get('ptc2_temp', 'n/a')}\n"
            f"  env={temp.get('env_temp', 'n/a')}  humidity={temp.get('env_humidity', 'n/a')}")
    ...
    def cmd_ACE_MATERIAL_INFO(self, gcmd: GCodeCommand) -> None:
        ...
        gcmd.respond_info(
            f"ACE material info slot {slot}: name={name!r} status={info.get('status', 0)}")
    ...
    def cmd_ACE_SET_MATERIAL(self, gcmd: GCodeCommand) -> None:
        ...
        try:
            ...
            gcmd.respond_info(
                f"ACE_SET_MATERIAL slot {slot}: stored name={stored!r}")
        except Exception as e:
            gcmd.respond_info(
                f"ACE_SET_MATERIAL slot {slot}: wrote {name!r} (read-back failed: {e})")
    ...
    def cmd_ACE_SENSOR_STATE(self, gcmd: GCodeCommand) -> None:
        ...
        gcmd.respond_info(
            f"ACE sensor state: mask=0x{mask:X} ({mask}) triggered channels={triggered}")
    ...
    def cmd_ACE_FEED_ASSIST(self, gcmd: GCodeCommand) -> None:
        ...
        if enable is None:
            error_str = "ENABLE is required (0 or 1)"
        ...
        if lane_name is not None:
            slot = self._slot_map.get(lane_name)
            if slot is None:
                error_str = f"Unknown lane for this unit: {lane_name}"
                raise gcmd.error(error_str)
        if slot is None:
            error_str = "LANE or SLOT is required"
            raise gcmd.error(error_str)
        if enable:
    ...
    def _maybe_assist_watchdog(self) -> None:
        ...
        if (slot is not None and lane is not None
            ...
            self.logger.info(
                f"ACE assist watchdog: enabling feed assist for {name} (slot {slot})")
            self.afc.reactor.register_callback(
                lambda et, n=name: self._reconcile_feed_assist(n))
    ...
    def _handle_stuck(self, cont_time: float) -> None:
        ...
        if slot is not None:
            try:
                ...
                pass
        lane_str = f" lane {name}" if name else ""
        msg = (
            f"ACE stuck spool detected on {self.name}{lane_str}: feed assist ran "
            f"continuously for {cont_time:.1f}s (>= {self._stuck_time:.1f}s threshold). "
            "The spool is likely tangled or jammed at the unit. Clear the snag, then "
            "resume. Run ACE_STUCK_SPOOL_DETECTION ENABLE=0 to disable this check.")
        try:
    ...
    def cmd_ACE_STUCK_SPOOL_DETECTION(self, gcmd: GCodeCommand) -> None:
        ...
        self.logger.info(
            f"ACE stuck spool detection {state}: stuck_time={self._stuck_time:.1f}s")
...
# ── ACE auto-discovery: serial_port: auto + ace_index ───────────────────────
# Chained V1 ACEs each enumerate as their OWN USB serial device, and their
# /dev/ttyACM* numbers shuffle on the firmware's ~3.5s watchdog re-enumeration,
# so a fixed ttyACM is useless. But the firmware self-reports a stable per-unit
# id in get_info -- {'result': {'id': N, ...}} -- that counts up by cable order
# (first unit id 1). We bind a config section to its unit by matching the
# section's `ace_index` to that id: scan the Anycubic serial devices, probe each
# with a one-shot get_info, and take the one whose id matches. Re-resolved on
# every (re)connect, so the ttyACM shuffle never mis-binds a unit. Explicit
# serial_port (by-id / by-path) skips all of this and pins the section to a port.
ACE_BYID_GLOB = "/dev/serial/by-id/*ANYCUBIC*-if00"

# Real tty paths already claimed by a live ACEConnection, so autodetect never
# probes (double-opens) a port another unit is using. Keyed by realpath.
_ACE_CLAIMED_PORTS: Set[str] = set()


def _ace_scan_candidates() -> List[str]:
    """Anycubic ACE serial devices currently present, as real tty paths."""
    out: List[str] = []
    for p in sorted(glob.glob(ACE_BYID_GLOB)):
        try:
...
def _ace_extract_get_info_id(buf: bytes) -> Optional[int]:
    """Pull result.id out of the first complete ACE frame in buf, or None."""
    i = buf.find(FRAME_HEADER)
    if i < 0 or len(buf) < i + 4:
        return None
    length = struct.unpack_from('<H', buf, i + 2)[0]
    end = i + 4 + length + 3           # header2 + len2 + payload + crc2 + footer1
    if len(buf) < end:
        return None
...
def probe_ace_get_info_id(port: str, baud: int = DEFAULT_BAUD,
                          timeout: float = 1.5) -> Optional[int]:
    """Open `port`, send one get_info, and return the firmware's self-reported
    unit id (result.id), or None. Standalone (no reactor) so it can bind a
    section to a unit before the real connection exists. Never raises."""
    try:
    ...
    try:
        ...
        while time.time() < deadline:
            ...
            if chunk:
                buf += chunk
                rid = _ace_extract_get_info_id(buf)
                if rid is not None:
                    return rid
            else:
                time.sleep(0.02)
...
def resolve_ace_port_by_index(ace_index: int, baud: int = DEFAULT_BAUD,
                              settle: float = 6.0,
                              logger: Optional[Any] = None) -> Optional[str]:
    """Return the tty whose ACE reports get_info id == ace_index, probing every
    Anycubic serial device over a settle window (to ride the ~3.5s watchdog
    re-enumeration). Ports already claimed by other live units are left alone."""
    deadline = time.time() + max(settle, 0.5)
    while True:
        for port in _ace_scan_candidates():
            if port in _ACE_CLAIMED_PORTS:
                continue
            rid = probe_ace_get_info_id(port, baud)
            if rid is not None and logger is not None:
                logger.info(f"ACE autodetect: {port} reports id {rid}")
            if rid == ace_index:
                return port
...
class ACEConnection:
    ...
    def __init__(self, reactor: Reactor, serial_port: str,
                 ...
                 ace_index: int = 1,
                 ace_uid: Optional[tuple] = None) -> None:
        ...
        :param reactor: Klipper reactor used for fd registration, timers and
            command completions.
        :param serial_port: Path to the ACE serial device, or the literal
            string 'auto' to resolve the port by firmware id (see ace_index).
        :param logger: Optional logger; defaults to the module serial logger.
        :param baud_rate: Serial baud rate.
        :param ace_index: When serial_port is 'auto', bind to the unit whose
            get_info id equals this (1-based, cable order).
        """
        # 'auto' resolves to a real tty on every connect via the unit's
        # firmware id; _resolved_port holds the last-resolved path for logs.
        self._auto = (isinstance(serial_port, str)
                      and serial_port.strip().lower() == "auto")
        self._ace_index = ace_index
        # Optional per-box pin (ACE2 only: the STM32 UID tuple from discover).
        self._ace_uid = ace_uid
    ...
    def connect(self) -> None:
        ...
        # Resolve serial_port: auto -> the tty whose firmware id == ace_index.
        # Done on every (re)connect so the ~3.5s ttyACM shuffle never mis-binds.
        port = self._resolve_connect_port()
        ...
        try:
            ...
            raise ACESerialError(
                f"Failed to open ACE serial port {port}: {e}"
            )

        # Claim the resolved tty so another unit's autodetect never double-opens
        # it. Tracked by realpath; released in disconnect().
        self._resolved_port = port
        with contextlib.suppress(Exception):
            _ACE_CLAIMED_PORTS.add(os.path.realpath(port))
        ...
        self._logger.info(
            f"ACE serial connected: {port} @ {self._baud_rate}"
            + (f" (auto, ace_index {self._ace_index})" if self._auto else "")
        )
    ...
    def _resolve_connect_port(self) -> str:
        """The tty to open. Explicit serial_port is returned as-is; 'auto'
        resolves to the unit whose firmware id == ace_index.

        Fast path on reconnect: re-check the last-resolved port first (one quick
        probe) so the common case doesn't run the full settle-window scan; only
        fall back to the scan if that unit has moved/renumbered."""
        if not self._auto:
            return self._serial_port
        if self._resolved_port:
            with contextlib.suppress(Exception):
                if probe_ace_get_info_id(
                        self._resolved_port, self._baud_rate) == self._ace_index:
                    return self._resolved_port
        port = resolve_ace_port_by_index(
            self._ace_index, self._baud_rate, logger=self._logger)
        if port is None:
            raise ACESerialError(
                f"ACE autodetect: no unit reporting id {self._ace_index} "
                f"found (serial_port: auto). Is it plugged in / powered?")
        return port
    ...
    def disconnect(self) -> None:
        ...
        # Release the claimed tty so autodetect can probe it again.
        if self._resolved_port is not None:
            with contextlib.suppress(Exception):
                _ACE_CLAIMED_PORTS.discard(os.path.realpath(self._resolved_port))
    ...
    def send_command(self, method: str, params: Optional[dict] = None,
        ...
        try:
            ...
            self.reconnect()
            error_str = f"ACE write failed: {e}"
            raise ACESerialError(error_str)
    ...
    def _handle_response(self, response: dict) -> None:
        ...
        if response_id is None:
            ...
            if self.status_callback:
                try:
                    ...
                    # Never silent: this dispatch is what a runout's whole
                    # TOOL_UNLOAD chain runs under, so a bare message here
                    # loses the only record of why it stopped.
                    self._logger.error(
                        f"ACE status_callback error: {e}\n"
        ...
        if self.status_callback:
            try:
                ...
                # Never silent: see the note on the unsolicited path above.
                self._logger.error(
                    f"ACE status_callback error: {e}\n"
                    f"{traceback.format_exc()}")
