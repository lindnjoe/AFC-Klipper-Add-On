...
def pb_varint(value: int) -> bytes:
    ...
    if value < 0:
        ...
        error_str = f"pb_varint cannot encode negative value {value}"
...
def method_to_v2(method: str,
    ...
    _logger.debug(f"ACE2: unknown method {method!r} -> GET_STATUS")
    return Cmd.GET_STATUS, b''
...
def encode_frame(request_id: int, cmd: int, payload: bytes = b'') -> bytes:
    ...
    if len(payload) > MAX_PAYLOAD_LEN:
        raise ValueError(
            f"V2 payload exceeds {MAX_PAYLOAD_LEN} bytes for opcode {cmd}")
    inner = bytearray([
        FLAG_REQUEST,
        seq & 0xFF, (seq >> 8) & 0xFF,
        cmd & 0xFF,
...
def v2_response_to_v1(cmd: int, seq: int, payload: bytes,
    ...
    try:
        ...
        # f-string, NOT printf args. `logger` here can be AFC's logger, whose
        # debug() is debug(message, only_debug, traceback) -- printf arguments
        # bind to those instead of formatting, so cmd became only_debug and the
        # exception became traceback, and the message printed its own %d.
        (logger or _logger).debug(
            f"ACE2 protobuf decode failure cmd={cmd}: {e}")
        return ret
    if cmd == Cmd.DISCOVER_DEVICE:
        ...
        # Surface any additional get_info protobuf fields the firmware sends
        # (e.g. a per-unit / chain id), so daisy-chain support can key off them
        # instead of silently dropping them. Numeric value if scalar.
        for k in sorted(fields):
            if k not in (1, 2):
                res[f'field{k}'] = _fval(fields, k)
        ret['result'] = res
        ...
        def _temp_range(fnum: int) -> Dict[str, Any]:
            """
            Pull a min/max temperature pair out of a nested protobuf field.

            :param fnum: field number holding the range message
            :return dict: {'min', 'max'}, empty when unset
            """
...
# ── ACE2 auto-discovery: serial_port: auto + ace_index (V2 probe) ───────────
# Same shape as V1's autodetect, but the probe speaks V2. Each ACE 2 is its own
# USB device (a CH34x adapter -- different serial-id naming from V1's ANYCUBIC,
# and unique per unit) whose ttyACM shuffles, so we find it by talking to it.
# ACE 2 get_info has NO per-unit id (unlike V1's result.id), so ace_index orders
# units by USB TOPOLOGY (cable order); the stable STM32 UID from discover_device
# both confirms a device IS an ACE 2 and lets a section optionally pin to a
# specific box (ace_uid). Only UNCLAIMED ports are probed, so a port another
# unit or MCU already holds open is never touched.
ACE2_BYID_GLOB = "/dev/serial/by-id/*1a86_USB_Single_Serial*-if00"


def _ace2_scan_candidates() -> List[str]:
    """CH34x serial devices currently present, as real tty paths."""
    out: List[str] = []
    for p in sorted(glob.glob(ACE2_BYID_GLOB)):
        try:
...
def _ace2_topology_key(real_tty: str) -> str:
    """Stable USB-topology sort key so 'cable order' is consistent across
    reboots: the /dev/serial/by-path link for this tty, with digit runs
    zero-padded so 2.10 sorts after 2.2 rather than lexically before it."""
    import re
    bypath = ""
    try:
        for p in glob.glob("/dev/serial/by-path/*"):
            with contextlib.suppress(Exception):
                if os.path.realpath(p) == real_tty:
                    bypath = os.path.basename(p)
                    break
    except Exception:
        pass
    return re.sub(r"\d+", lambda m: m.group().zfill(6), bypath or real_tty)


def _ace2_extract_uid(buf: bytearray):
    """Pull (uid1,uid2,uid3) from the first complete discover RESPONSE in buf,
    or None."""
    i = buf.find(PREAMBLE)
    if i < 0 or len(buf) < i + HEADER_LEN:
        return None
    flags = buf[i + 2]
    cmd = buf[i + 5]
    payload_len = buf[i + 6]
    end = i + HEADER_LEN + payload_len + TRAILER_LEN
    if len(buf) < end:
        return None
    if not (flags & FLAG_RESPONSE) or cmd != Cmd.DISCOVER_DEVICE:
        return None
    payload = bytes(buf[i + HEADER_LEN:i + HEADER_LEN + payload_len])
    try:
        fields = pb_decode(payload)
        u1 = _fval(fields, 1)
        if u1 is None:
            return None
        return (int(u1), int(_fval(fields, 2) or 0), int(_fval(fields, 3) or 0))
    except Exception:
        return None


def probe_ace2_uid(port: str, baud: int, timeout: float = 1.5):
    """Open `port`, send one V2 discover_device, and return the ACE 2's 96-bit
    STM32 UID as (uid1, uid2, uid3), or None if it is not a live ACE 2.
    Standalone (no reactor); never raises."""
    try:
    ...
    try:
        ...
        ser.write(encode_frame(0, Cmd.DISCOVER_DEVICE))
        buf = bytearray()
...
def resolve_ace2_port(ace_index: int, baud: int, ace_uid=None,
                      settle: float = 6.0, logger=None) -> Optional[str]:
    """The tty for the ACE 2 this section wants. Probes CH34x devices over a
    settle window (rides the watchdog re-enumeration). With ace_uid, binds to
    the box reporting that UID; otherwise binds ace_index (1-based) by
    USB-topology (cable) order. Claimed ports are skipped."""
    deadline = time.time() + max(settle, 0.5)
    found: Dict[str, tuple] = {}
    while True:
        for port in _ace2_scan_candidates():
            if port in _ACE_CLAIMED_PORTS or port in found:
                continue
            uid = probe_ace2_uid(port, baud)
            if uid is not None:
                found[port] = uid
                if logger is not None:
                    # f-string: this logger is AFC's whenever no serial log
                    # file is configured (_create_serial_logger returns None
                    # and the caller falls back to self.logger), and AFC's
                    # info() is info(message, console_only) -- three
                    # positionals is a TypeError, in the middle of autodetect.
                    logger.info(f"ACE2 autodetect: {port} -> uid {uid}")
                if ace_uid is not None and uid == tuple(ace_uid):
                    return port
        if time.time() >= deadline:
            break
        time.sleep(0.5)
    if ace_uid is not None:
        return next((p for p, u in found.items() if u == tuple(ace_uid)), None)
    if found:
        ordered = sorted(found, key=_ace2_topology_key)
        if 1 <= ace_index <= len(ordered):
            return ordered[ace_index - 1]
    return None
...
class ACE2Connection(ACEConnection):
    ...
    def _resolve_connect_port(self) -> str:
        """ACE2 override: resolve serial_port: auto with the V2 discover probe
        (V1's uses a JSON get_info). Cache-first on reconnect -- re-check the
        last port before the full settle-window scan."""
        if not self._auto:
            return self._serial_port
        if self._resolved_port:
            with contextlib.suppress(Exception):
                uid = probe_ace2_uid(self._resolved_port, self._baud_rate)
                if uid is not None and (self._ace_uid is None
                                        or uid == tuple(self._ace_uid)):
                    return self._resolved_port
        port = resolve_ace2_port(self._ace_index, self._baud_rate,
                                 ace_uid=self._ace_uid, logger=self._logger)
        if port is None:
            raise ACESerialError(
                f"ACE2 autodetect: no unit found for ace_index "
                f"{self._ace_index}"
                + (f" / ace_uid {self._ace_uid}" if self._ace_uid else "")
                + " (serial_port: auto). Is it plugged in / powered?")
        return port
    ...
    def send_command(self, method: str, params: Optional[Dict[str, Any]] = None,
        ...
        try:
            ...
            error_str = f"ACE2 encode failed for '{method}': {e}"
            raise ACESerialError(error_str)
        ...
        try:
            ...
            self.reconnect()
            error_str = f"ACE2 write failed: {e}"
            raise ACESerialError(error_str)
...
class afcACE2(afcACE):
    ...
    def __init__(self, config: ConfigWrapper) -> None:
        ...
        # Optional per-box pin for serial_port: auto -- the STM32 UID the unit
        # reports from discover_device, as three comma-separated integers
        # (uid1,uid2,uid3, exactly as they appear in AFC_ACE_serial.log). When
        # set, this section binds to THAT physical box regardless of cable
        # order; when omitted, ace_index picks by USB-topology (cable) order.
        self.ace_uid: Optional[tuple] = None
        _uid = config.get("ace_uid", None)
        if _uid:
            parts = tuple(p for p in _uid.replace(",", " ").split() if p)
            try:
            ...
            if len(nums) != 3:
                raise config.error(
                    f"[{config.get_name()}] ace_uid must be three integers "
                    f"'uid1,uid2,uid3' (copy them from the discover line in "
                    f"AFC_ACE_serial.log); got '{_uid}'")
            self.ace_uid = nums
        ...
        if self.feed_check_length >= _expected:
            raise config.error(
                (f"[{config.get_name()}] feed_check_length ({self.feed_check_length}) must be < "
                    f"feed_error_length * {ACE2_ENCODER_SCALE:.4f} = {_expected:.0f}, otherwise "
                    f"the encoder can never reach it and every feed raises FEED_ERROR. Lower "
                    f"feed_check_length to widen tolerance (tolerance_mm ~= feed_error_length - "
                    f"feed_check_length / {ACE2_ENCODER_SCALE:.4f})."))
        # Stuck/tangle detection defaults on for the ACE2: its filament encoder reports
        # a per-slot error state (stuck/tangled/assist/motor) when motor steps and
        # measured movement diverge, a true jam signal, so _check_stuck below reacts to
        # those instead of the V1 assist-duration proxy.
    ...
    def _apply_feed_check(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                (f"ACE2 {self.name}: feed check set check_length={self.feed_check_length} "
                    f"error_length={self.feed_error_length}"))
        except Exception as e:
            self.logger.warning(
                f"ACE2 {self.name}: set_feed_check failed (non-fatal): {e}")
    ...
    def _handle_encoder_jam(self, name: str, slot: int, state: str) -> None:
        ...
        msg = (
            f"ACE2 {self.name} lane {name}: {pretty}. The unit's filament encoder reports "
    ...
    def _make_connection(self, reactor: Reactor, serial_port: str,
        ...
        return ACE2Connection(reactor=reactor, serial_port=serial_port,
                              ...
                              ace_index=self.ace_index, ace_uid=self.ace_uid)
