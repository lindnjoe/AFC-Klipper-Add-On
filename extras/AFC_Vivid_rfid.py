...
class AFC_Vivid_rfid(AFCUnitRFID):
    ...
    def _stage_read_begin(self, lane: "AFCLane") -> None:
        ...
        try:
            ...
            if sib_lane is not None and getattr(sib_lane, "prep_state", False):
                ...
                # WHOSE TAG IS IT? A shared antenna is shared in both
                # directions: this slot's own spool is stationary too, so its
                # tag can be the one resting on the coil. Seen on a BoxTurtle
                # with the same logic -- the active lane's own tag read as
                # parked and the sibling was moved for a collision that did
                # not exist. _slot_uid is the evidence, and comparing against
                # it is what ACE2's _is_sibling_tag does.
                #
                # An unknown UID stays a collision on purpose: that is the
                # first-encounter case this clear exists for, and guessing
                # "probably ours" there is the silent two-slots-one-spool
                # failure.
    ...
    def _retract_sibling(self, p: dict) -> None:
        ...
        if getattr(sib_lane, "tool_loaded", False) or self._afc_is_printing():
            ...
            return
        # prep_state only says filament is touching the prep switch, which the
        # operator's hand can do before the gears have taken hold. The LOAD
        # switch is where a load actually homes the tip, so it is the one that
        # says there is filament to give back. Without this check a blind
        # retract walks a badly staged lane out of itself -- which is exactly
        # what it did on a BoxTurtle before the same gate went in there.
        ...
        # Stepped and watched, so the distance is a MAXIMUM rather than a
        # commitment: a properly staged sibling runs the full distance, a
        # badly staged one stops the moment it would come off its switch.
        step = getattr(sib_lane, "short_move_dis", None) or 10.0
        moved = 0.0
        while moved < want and self._seated(sib_lane):
            hop = min(step, want - moved)
            # Assist ON: this is the retract, and without the spooler
            # winding it back the filament it pulls piles up loose on the
            # reel rather than onto it. Feeding is the direction that
            # free-wheels and does not want help.
    ...
    def _restore_sibling(self, p: Optional[dict]) -> None:
        ...
        NOT a blind re-feed of the distance taken. That only lands right if
        neither move slipped, and they do: the gears skid against a spool that
        has not started turning, so the give-back under-delivers and the error
        is one-directional. Repeated over several reads it walks the tip
        backwards out of the lane. The BoxTurtle hit exactly this.

        So the SWITCH is the answer, asked through AFC's own homing move --
        the same one a load and an unload make, where the MCU stops the move
        on the endstop rather than a Python loop noticing between chunks.
        Without homing it falls back to the give-back plus a stepped creep.

        NOTE WHAT THIS DOES AND DOES NOT FIX. A normally staged sibling sits
        well past its load switch, so the switch never releases and the creep
        never runs -- the shortfall from slip is real and is not corrected
        here. There is nothing downstream to measure against: the BoxTurtle
        re-stages against its hub, and a ViViD has no equivalent. What this
        does catch is the case that actually damages something, a sibling
        walked far enough back that it comes out of the lane, and it says so
        out loud -- off its own load switch it reads as "spool removed", and
        the operator needs to know it was this that moved it.
        ...
        try:
            # Give back what was taken. This has to be a plain move: a
            # forward homing move stops the instant the load switch reads
            # made, and a normally staged sibling never released it, so
            # homing here would travel zero and the sibling would keep the
            # 75mm it was moved by.
            ...
            if self._seated(sib_lane):
                ...
                # Only now is homing the right tool: the switch IS released,
                # so a forward home has somewhere to travel and stops the
                # moment it is made. Same motion prep_load uses on a load.
