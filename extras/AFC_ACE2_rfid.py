...
class AFC_ACE2_RFID(AFCUnitRFID):
    ...
    def _register_writers(self) -> None:
        ...
        Per SLOT rather than per reader pair: a blank sticker is held against
        one slot's antenna, and the reg chip-select is per-slot on this
        firmware, so the slot is what the caller actually means.

        The prepare/release pair mirrors the managed read exactly -- stop the
        firmware's identify loop on BOTH slots of the pair (it power-gates the
        reader and soft-resets it every cycle, which would tear a multi-page
        write apart), take the power, and put both back afterwards.
        """
        ...
        def _open(slot):
            ...
            if self.ace2 is None:
                return None
            return _Ace2RegLink(self.ace2, self._reg_index(slot) & 0xFF,
                                power_index=(slot >> 1) & 0xFF,
                                reg_timeout=self.read_reg_timeout)

        def _prepare(link, slot):
            """
        ...
        def _release(link, slot):
            """
            Hand the reader back. Best-effort: a wedged serial here must not
            mask the write's own result.
            """
            try:
    ...
    def _on_ready(self) -> None:
        ...
        # AFC's logger from here: AFC.log with timestamps and call sites,
    ...
    def read_lane(self, lane_name: str) -> Optional[dict]:
        ...
        if slot is None:
            raise self.printer.command_error(
                f"lane {lane_name!r} has no ACE2 reader slot (set lane_slot_map)")
    ...
    def cmd_ACE_RFID_STAGE_TEST(self, gcmd: "GCodeCommand") -> None:
        ...
        if conn is None or not getattr(conn, "connected", False):
            error_str = "STAGE_TEST: ACE serial not connected"
            raise gcmd.error(error_str)
        ...
        def _ready(where: str) -> None:
            ...
            """
            Wait for the unit to report ready between stage steps.

            :param where: step name for the log line
            """
            if callable(wait_ready):
                try:
                    if not wait_ready(timeout=self.read_reg_timeout + 20.0):
                        log(f"STAGE_TEST: (unit not ready after {where})")
                except Exception:
                    pass

        log(f"STAGE_TEST {lane_name} (slot {slot}): retracting {dist:.0f}mm to make room...")
        try:
            conn.unwind_filament(slot, dist, rspeed)
        except Exception as e:
            error_str = f"STAGE_TEST: initial retract failed: {e}"
            raise gcmd.error(error_str)
        _ready("retract")
        ...
        log(f"STAGE_TEST: feeding {dist:.0f}mm forward while polling the reader...")
        feed_ok = True
        t0 = self.reactor.monotonic()
        try:
            conn.feed_filament(slot, dist, speed)
            ...
            feed_ok = False
            log(f"STAGE_TEST: feed returned/err: {e}")
        ...
        if feed_ok:
            ...
            while self.reactor.monotonic() < deadline:
                ...
                if uid is not None:
                    ...
                    log(f'STAGE_TEST: poll #{state["polls"]} DETECTED uid={uid.hex()} -> STOP_FEED '
                        f'(~{state["moved"]:.0f}mm in)')
                    try:
                        conn.stop_feed_filament(slot)
                    except Exception as e:
                        log(f"STAGE_TEST: stop_feed error: {e}")
                    break
            ...
            if not state["stopped"] and state["moved"] == 0.0:
                state["moved"] = dist
        log(f'STAGE_TEST: feed ended, feed_ok={feed_ok} polls={state["polls"]} detected='
            f'{state["detected"]} stopped={state["stopped"]} moved=~{state["moved"]:.0f}mm')

        # Watchdog / stuck-spool check (best-effort).
        checker = getattr(self.ace2, "_slot_in_error", None)
        if callable(checker):
            try:
                log(f"STAGE_TEST: slot {slot} in error (watchdog)? {bool(checker(slot))}")
            except Exception:
                pass
        ...
        if state["detected"]:
            def _try_read() -> Optional[dict]:
                """
                One tag read attempt; errors are logged and become None.
            ...
            if not tag:
                ...
                while nudge_total < 42.0 and not tag:
                    try:
                        ...
                        log(f"STAGE_TEST: nudge error: {e}")
                        break
                    nudge_total += step
                    self._dwell(0.15)
                    tag = _try_read()
                    if tag:
                        log(f"STAGE_TEST: re-centered after {nudge_total:.0f}mm retract")
            ...
            log(f'STAGE_TEST: parked read decoded={decoded} -> {(tag or {}).get("filament")}')
        else:
        ...
        if remaining > 1.0:
            _ready("pre-restage")
            log(f"STAGE_TEST: re-feeding {remaining:.0f}mm to re-stage the lane...")
            try:
                conn.feed_filament(slot, remaining, speed)
            except Exception as e:
                log(f"STAGE_TEST: re-stage feed err: {e}, re-stage lane1 manually!")
    ...
    def cmd_ACE_RFID_SCAN(self, gcmd: "GCodeCommand") -> None:
        ...
        if self._scan_running:
            raise self.printer.command_error(
                "ACE2 RFID scan: a scan is already running")
        gcmd.respond_info(
    ...
    def _scan_in_background(self, lane_name: str, seconds: float) -> None:
        ...
        try:
            ...
            # A serial timeout / reader glitch must NEVER shut Klipper down --
            # and out here there is no command left to raise to, so even a
            # command_error is reported rather than thrown.
