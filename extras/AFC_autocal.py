...
class AFC_autocal:
    ...
    def _patch_set_tool_loaded_emit(self) -> None:
        ...
        The wrapper forwards ``*args``/``**kwargs`` verbatim rather than naming
        upstream's parameters. Restating a frozen file's signature here means an
        upstream that adds an argument crashes Klipper from inside OUR wrapper,
        which is exactly how the set_spoolID patch below broke -- see the note
        there.
        """
        try:
            from extras.AFC_lane import AFCLane
        ...
        def set_tool_loaded(self: AFCLane, *args: Any,
                            _orig: Callable = _orig, **kwargs: Any) -> None:
            """
            Wrapped AFCLane.set_tool_loaded: run the original, then announce the load.

            :param _orig: the original method
            :param args: forwarded to the original
            :param kwargs: forwarded to the original
            """
    ...
    def _patch_set_spoolid_emit(self) -> None:
        ...
        TWO THINGS THIS GOT WRONG, both from the same cause -- the wrapper
        restated upstream's signature instead of forwarding it.

        Upstream made the Spoolman fetch async and gave set_spoolID an
        ``on_done`` callback for "the lane's data is now actually applied". The
        old wrapper declared ``(self, cur_lane, SpoolID, save_vars=True)``, so:

        1. A SET_SPOOL_ID that passed ``on_done=`` -- which cmd_SET_SPOOL_ID now
           always does -- raised TypeError from inside this wrapper and SHUT
           KLIPPER DOWN. Observed on hardware, mid-print, from a U1 RFID scan.
        2. Even without that, emitting straight after ``_orig`` returned was
           emitting too EARLY: with Spoolman configured, set_spoolID now returns
           as soon as the fetch is scheduled, before ``_apply_spool_data`` has
           set ``cur_lane.spool_id``. _do_spool_assigned reads exactly that
           field and bails when it is None, so a staged spool silently never
           got calibrated.

        So bind against upstream's REAL signature and chain the event onto
        ``on_done``. That fixes the ordering (the event now means what its name
        claims) and the arity, and upstream's own ``finally`` guarantees
        on_done fires on every exit path -- success, bad weight, or exception --
        so the event cannot be lost. A caller's own on_done still runs first,
        and still propagates its exceptions.
        """
        try:
            from extras.AFC_spool import AFCSpool
        except Exception as e:
        ...
        try:
            _sig: Optional[inspect.Signature] = inspect.signature(_orig)
        except (TypeError, ValueError):
            _sig = None
        # Whether upstream still has the completion callback to chain onto. If a
        # later upstream drops it, fall back to emitting on return -- the old
        # behaviour, which is right again the moment the call is synchronous.
        _chainable = _sig is not None and 'on_done' in _sig.parameters

        def set_spoolID(self: AFCSpool, *args: Any, _orig: Callable = _orig,
                        _sig: Optional[inspect.Signature] = _sig,
                        **kwargs: Any) -> None:
            """
            Wrapped AFCSpool.set_spoolID: run the original, then announce the assignment.

            :param _orig: the original method
            :param _sig: the original's signature, for resolving the lane argument
            :param args: forwarded to the original
            :param kwargs: forwarded to the original
            """
            def emit(cur_lane: Any) -> None:
                """Announce the assignment; never let a handler break the call."""
                try:
            ...
            if not _chainable:
                _orig(self, *args, **kwargs)
                emit(args[0] if args else kwargs.get('cur_lane'))
                return
            try:
                bound = _sig.bind(self, *args, **kwargs)
            except TypeError:
                # Not a call upstream accepts either. Let IT raise the error, so
                # the traceback names the real signature and not this wrapper.
                _orig(self, *args, **kwargs)
                return
            cur_lane = bound.arguments.get('cur_lane')
            caller_done = bound.arguments.get('on_done')

            def chained() -> None:
                """Caller's completion first, then ours -- ours regardless."""
                try:
            ...
            bound.arguments['on_done'] = chained
    ...
    def _make_pa_wrapper(self, original: Callable, name: str) -> Callable:
        ...
        def wrapper(gcmd: GCodeCommand) -> None:
            """
            Ignore slicer PA for a managed extruder while printing, else run the original.

            :param gcmd: the gcode command
            """
    ...
    def _check_staged_k_async(self, lane_name: str, sid: int) -> None:
        ...
        def worker() -> None:
            """
            Background: read the staged spool's K, then hop back to the reactor.
            """
            try:
        ...
        threading.Thread(target=worker, name="afc_autocal_stg",
                         daemon=True).start()
    ...
    def _fetch_k_async(self, cur_lane: Any) -> None:
        ...
        def worker() -> None:
            """
            Background: read the spool's K, then hop back to the reactor to apply it.
            """
            try:
                thread_name = threading.current_thread().name
                chelper.get_ffi()[1].set_thread_name(thread_name.encode("utf-8"))
        ...
        threading.Thread(target=worker, name="afc_autocal_k",
                         daemon=True).start()
