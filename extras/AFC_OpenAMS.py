...
class afcAMS(afcUnit):
    ...
    def _sync_lanes_from_hardware(self) -> None:
        ...
        for lane_name, lane in self.lanes.items():
            ...
            # filament is at the junction, 0 at idle) — that feeds the virtual
            # hub's any(raw_load_state), and is a different thing from the
            # lane's own load state. A bay has one switch, so prep and load
            # both come from F1S: loaded_to_hub (which AFCLane.load_state
            # returns on a virtual hub) means the spool is staged just short
            # of the hub sensor, and the firmware stages every seated spool.
            # _update_f1s_debounced keeps the pair in step from here on.
    ...
    def _update_f1s_debounced(self, lane: AFCLane, lane_name: str, slot: int,
        ...
        """
        Debounced f1s handling for one lane.

        A bay has ONE switch, so prep and load are the same signal and this
        sets both from it. On a virtual hub AFCLane.load_state returns
        loaded_to_hub (AFC_lane.py), so loaded_to_hub is the lane's load
        state here — and a bay with a spool in it has been auto-staged by
        the firmware, which is what loaded_to_hub means: filament parked
        just short of the hub sensor, not the hub sensor reading filament.
        Nothing on the host can un-stage a seated spool, so f1s is the whole
        truth and this re-asserts it rather than only clearing on removal.

        That matters because AFC.LANE_UNLOAD clears loaded_to_hub after
        calling eject_lane, and OpenAMS cannot eject — eject_lane declines,
        the spool stays seated and staged, and the flag is cleared anyway.
        The lane was left prep=True/load=False, which for a shared switch is
        not a state the hardware can be in; the UI showed "filament detected
        but not loaded" and only pulling the spool for a fresh insert edge
        cleared it. Re-asserting here heals it on the next poll.

        A flaky/threshold-riding f1s chatters 0/1 rapidly; acting on raw
        edges would fire runouts and insert scans on noise, so a state change
        only commits after holding for ``f1s_debounce`` seconds — until then
        the lane keeps the last committed state.
        ...
        if resync_prev or committed is None:
            ...
            lane.prep_state = lane.loaded_to_hub = raw_f1s
            return

        if raw_f1s == committed:
            # Stable (or chattered back) — drop any pending change.
            self._f1s_pending_since[slot] = None
            lane.prep_state = lane.loaded_to_hub = committed
            return
    ...
    def _oams_load(self, cur_lane: AFCLane, max_retries: int = 3) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                #
                # force: _wait_for_idle() has just waited for the unit to
                # ANSWER, and answering is what clears action_status -- so an
                # unforced abort here finds nothing tracked and returns without
                # sending the firmware cancel. That is the whole point of this
                # cleanup: it runs after a failed attempt, when the unit is
                # most likely still driving filament, and it was the one call
                # that could never have stopped it.
                self._wait_for_idle()
                self.oams.abort_current_action(wait=True, force=True)
                self._wait_for_idle()
                self.oams.unload_spool_with_retry()
                self._wait_for_idle()
    ...
    def cmd_AFC_OAMS_CLEAR_ERRORS(self, gcmd: GCodeCommand) -> None:
        ...
        try:
            # force: this is the command an operator reaches for when the unit
            # is wedged, and a wedged unit has already answered -- so
            # action_status is clear and an unforced abort would send no
            # firmware cancel at all. AFC_OAMS_CLEAR_ERRORS would then clear
            # the HOST's idea of the error while the unit carried on doing
            # whatever it was doing, which is the opposite of what its name
            # promises.
            self.oams.abort_current_action(force=True)
    ...
    def _rfid_read_stationary(self, coord: Any, rfid_slot: int,
        ...
        for i in range(attempts):
            ...
            if tag and tag.get("uid") and getattr(self, "_write_test", False):
                # Armed diagnostic: the tag is stationary at the antenna now, so
                # run the Classic write test here where it can reach it.
                self._write_test = False
                try:
                    self._log_write_test(coord, rfid_slot)
                except Exception as e:
                    self.logger.info(f"OAMS: write test failed: {e}")
    ...
    def _log_write_test(self, coord: Any, rfid_slot: int) -> None:
        """
        Run the Classic write test on the positioned tag and log the result.

        Proves the 0xA0 block write works on real data and that the tag is left
        exactly as found.

        :param coord: the AFC_OpenAMS_rfid coordinator
        :param rfid_slot: physical RFID slot with the tag at the antenna
        """
        res = coord.classic_write_test_slot(rfid_slot)
        if not isinstance(res, dict) or res.get("error"):
            self.logger.info(
                f"OAMS write test: {(res or {}).get('error', 'no result')}")
            return
        ok = res["wrote"] and res["changed"] and res["restored"]
        verdict = "WORKS" if ok else "did NOT fully work"
        self.logger.info(
            f"OAMS Classic write test -- uid {res['uid']} block {res['block']}\n"
            f"  access bits     {res.get('access')}\n"
            f"  orig            {res['orig']}\n"
            f"  wrote flipped   ack={res['wrote']} changed={res['changed']} "
            f"-> {res['read_after_write']}\n"
            f"  restored orig   ack={res['restored_ack']} "
            f"restored={res['restored']} -> {res['read_after_restore']}\n"
            f"  VERDICT: 0xA0 write {verdict}")
    ...
    def cmd_AFC_OAMS_RFID_SCAN(self, gcmd: GCodeCommand) -> None:
        ...
        self._rfid_scanned.discard(lane_name)
        # WRITETEST=1: run the Classic write test at the stationary-read
        # position (proves 0xA0 works on a real tag, then restores it).
        self._write_test = bool(gcmd.get_int("WRITETEST", 0))
