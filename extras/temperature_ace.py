...
#
#   [temperature_sensor ace_dryer_ptc1]
#   sensor_type: temperature_ace
#   ace_unit: Ace1
#   channel: ptc1            # default | box1 | box2 | ptc1 | ptc2 | env
#   min_temp: 0
#   max_temp: 120
#
# channel defaults to 'default' (the single box/env temp from get_status,
# unchanged from before). The other channels are ACE2-only.
...
class TemperatureACE:
    ...
    def __init__(self, config: "ConfigWrapper") -> None:
        ...
        # Which thermal channel to report. 'default' keeps the historical
        # behaviour (the single box/env temp from get_status). The other choices
        # read the richer GET_TEMP channels the unit caches each heartbeat
        # (ACE2 only): box1/box2 (chamber), ptc1/ptc2 (dryer heater elements),
        # env (enclosure). Lets you add a dedicated sensor per channel, e.g. a
        # ptc1 sensor to watch the dryer heater during a drying cycle.
        self.channel = config.getchoice(
            "channel",
            {c: c for c in ("default", "box1", "box2", "ptc1", "ptc2", "env")},
            "default")
        ...
        # the humidity field (ACE 2). Mainsail reads the humidity object
        # directly; Fluidd maps the temperature_sensor's sensor_type
        # (aht2x -> aht10) to this object. For Fluidd, set sensor_type: aht2x
        # (alias registered by AFC_ACE). Falls back to the plain name when
    ...
    def _sample_ace_temperature(self, eventtime: float) -> float:
        """
        ...
        try:
            ...
            if self._ace_unit:
                ...
                if self.channel == "default":
                    # Historical behaviour: the single box/env temp + humidity
                    # from get_status.
                    self.temp = float(hw_status.get("temp", 0.0) or 0.0)
                    if "humidity" in hw_status:
                        self._has_humidity = True
                        self.humidity = float(hw_status.get("humidity", 0.0) or 0.0)
                else:
                    # A specific GET_TEMP channel (ACE2). env_humidity travels
                    # with the same cache, so surface it too.
                    temp_info = getattr(self._ace_unit, "_cached_temp_info", {})
                    self.temp = float(
                        temp_info.get(self.channel + "_temp", 0.0) or 0.0)
                    if "env_humidity" in temp_info:
                        self._has_humidity = True
                        self.humidity = float(
                            temp_info.get("env_humidity", 0.0) or 0.0)
                ...
                if self.temp > 0 and self.temp < self.min_temp:
                    self.printer.invoke_shutdown(
                        f"ACE temperature {self.temp:.1f} below minimum of {self.min_temp:.1f}"
                    )
                if self.temp > self.max_temp:
                    self.printer.invoke_shutdown(
                        f"ACE temperature {self.temp:.1f} above maximum of {self.max_temp:.1f}"
                    )
            else:
                self.temp = 0.0
    ...
    def stats(self, eventtime: float) -> Tuple[bool, str]:
        ...
        """
        return False, f"temperature_ace {self.name}: temp={self.temp:.1f}"
...
def _register_sensor_factory(printer: "Printer") -> None:
    """
    ...
    # Register the Fluidd alias here too (early), not only in AFC_ACE: a plain
    # [temperature_sensor] sensor_type: aht2x is resolved in the main config pass,
    # before the AFC unit (which loads later) would register it, registering it
    # only there raises "Unknown temperature sensor 'aht2x'" at boot.
    heaters.add_sensor_factory("aht2x", TemperatureACE)
    _REGISTERED = True