...
class AFC_U1_RFID:
    ...
    def __init__(self, config: "ConfigWrapper") -> None:
        ...
        # AFC'S LOGGER FROM CONSTRUCTION, not from klippy:ready.
        #
        # load_object CONSTRUCTS AFC if this section is reached first, which is
        # how AFC_lane, AFC_buffer and AFC_extruder all take it. The stdlib
        # logger this used to hold until the swap only reached klippy.log, so
        # anything logged before ready missed AFC.log and the console -- and
        # the stdlib rejects AFC's keywords, so those calls had to stay plain.
        #
        # self.afc stays None until klippy:ready deliberately: it is the READY
        # MARKER for this module, and a dozen guards read it that way.
        ...
        # DEFAULT 0 (opt-in): ONLY set this (~1.0) when you've actually configured
        # the OpenRFID webhook exporter. Without a webhook the deferral buys
        # nothing and just delays every scan by this much (and the defer/confirm
        # re-fire can be tripped by an intermittent read or a quick tag removal),
        # so leaving it on by default would break the plain filament_detect
        # scanner path. Once a webhook is seen the filament_detect read is
        ...
        # default (a hidden dir beside the config, matching the path the
        # shipped write-watch config points the daemon at) is resolved lazily
        # in _write_dir, so construction stays free of a printer lookup.
    ...
    def _patch_scanner_rfid_update(self) -> None:
        ...
        def patched_rfid_cb(*args: Any, **kwargs: Any) -> None:
            ...
            THE SIGNATURE IS DELIBERATELY NOT SPELLED OUT. This function is
            called by the U1's own firmware, not by us, so naming its arguments
            here means a Snapmaker update that adds one raises TypeError from
            inside our wrapper -- and an exception on that path shuts Klipper
            down. The autocal set_spoolID patch cost a mid-print shutdown
            learning exactly this. Only the channel is read, and only because
            it is the first argument on every version of this API seen so far;
            if it ever is not, the isinstance check falls through to passing
            everything on untouched, which is the safe direction.
    ...
    def _write_dir(self) -> str:
        """Where write requests are dropped for the OpenRFID daemon to pick up.

        A directory both sides can reach: Klipper (this user) writes requests,
        the root daemon writes results. It sits under the config directory, so
        it rides the same path both already share, and can be overridden with
        ``openrfid_write_dir`` when the daemon is pointed elsewhere.
        """
    ...
    def _register_writers(self) -> None:
        ...
        file the daemon answers. Registered per channel, named for what the
        channel is -- a standalone scanner, or the lane it feeds.
        """
        seen: set = set()
    ...
    def _check_channel(self, lane_name: Optional[str], channel: int,
        ...
        def _after_sync() -> None:
            """Runs on the reactor once the lane actually carries the spool."""
    ...
    def _map_to_slot_info(self, info: dict) -> dict:
        ...
        # Optional rich fields, uniform with the host-side readers
        # (map_tag_to_slot_info) — only when the tag/daemon carried them.
        if ext_min:
    ...
    def _fmt_uid(raw: Any) -> Optional[str]:
        ...
        if isinstance(raw, (list, tuple)):
            try:
                ...
                return None
        # A hex string (e.g. from the OpenRFID webhook's scan.uid) — uppercase so
        # it matches the byte-list form filament_detect sends for the same tag.
    ...
    def _notify_scan(self, brand: str, material: str, color: str,
        ...
        try:
            ...
            # Enrich from the LANE, which set_spoolID has just populated from
            # its own fresh Spoolman fetch.
            #
            # This used to call enrich_from_spool, which is another blocking
            # GET -- on the reactor, on every scan, on top of the round trips
            # sync_rfid_to_spoolman had already made. Reading the lane costs
            # nothing, and it is not a cached snapshot either: it is the
            # newest read in the system, and the one that was actually
            # applied, so the popup cannot disagree with the lane it
            # describes. We get here through on_done, which is what guarantees
