...
try: from extras.AFC_RFID import (
    build_filament_name,
    get_auto_spoolman_create, apply_filament_defaults,
    sync_rfid_to_spoolman, enrich_from_spool, SpoolmanClient,
    make_tag_record,
    )
...
class AFC_U1_RFID:
    ...
    def _patch_scanner_rfid_update(self) -> None:
        ...
        def patched_rfid_cb(channel: int, info: dict,
                            is_clear: bool = False) -> None:
    ...
    def _notify_scan(self, brand: str, material: str, color: str,
        ...
        try:
            ...
            # Enrich from the matched Spoolman spool so the read-out shows the
            # stored record (name/material/temps) when the lane is bound to one.
            mr = getattr(self.afc, "moonraker", None)
            if spool_id and mr is not None:
                enriched = enrich_from_spool(SpoolmanClient(mr), spool_id,
                                             slot_info)
                name = enriched.get("display_name") or name
                brand = enriched.get("brand") or brand
                material = enriched.get("material") or material
                ext = enriched.get("extruder_temp") or ext
                bed = enriched.get("bed_temp") or bed
